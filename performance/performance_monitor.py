...
class PerformanceMonitor:
    ...
    @contextmanager
    def monitor_operation(self, operation_name: str = "operation") -> ContextManager[PerformanceMetrics]:
        """
    ...
    def benchmark_function(self, func: Callable, *args, iterations: int = 1, 
                          warmup_iterations: int = 0, **kwargs) -> List[PerformanceMetrics]:
        """
...
def monitor_memory_usage(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            logger.info(f"{func.__name__} memory usage: {memory_delta:+.2f}MB (final: {final_memory:.2f}MB)")
            return result
        except Exception as e:
            final_memory = process.memory_info().rss / 1024 / 1024
            memory_delta = final_memory - initial_memory
