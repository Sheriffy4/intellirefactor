...
class DatabaseOptimizer:
    ...
    def _analyze_query_patterns_for_table(self, table_name: str) -> Dict[str, Any]:
        ...
        table_stats = [stat for stat in self._performance_stats 
                      if table_name.lower() in stat.query.lower()]
        ...
        if table_stats:
            patterns['avg_execution_time'] = sum(s.execution_time for s in table_stats) / len(table_stats)
        
        return patterns
    
    def _generate_optimization_suggestions(self, table_name: str, stats: Dict[str, Any]) -> List[str]:
        ...
        if row_count > 1000 and index_count < 2:
            suggestions.append(f"Consider adding indexes to table '{table_name}' with {row_count} rows")
        
        # Suggest composite indexes for frequent joins
        if query_patterns.get('select_queries', 0) > query_patterns.get('insert_queries', 0) * 2:
            suggestions.append(f"Table '{table_name}' is read-heavy, consider optimizing for SELECT queries")
        
        # Suggest partitioning for very large tables
        if row_count > 100000:
            suggestions.append(f"Table '{table_name}' is very large ({row_count} rows), consider partitioning strategies")
        
        # Suggest VACUUM for tables with many modifications
        update_ratio = query_patterns.get('update_queries', 0) + query_patterns.get('delete_queries', 0)
        if update_ratio > 100:
            suggestions.append(f"Table '{table_name}' has many modifications, consider running VACUUM")
        
        return suggestions
    
    def create_optimized_indexes(self, table_name: str, column_combinations: List[List[str]]) -> List[str]:
        """
    ...
    def execute_with_performance_tracking(self, query: str, params: Optional[Tuple] = None) -> QueryPerformanceStats:
        """
        ...
        try:
            ...
            logger.debug(f"Query executed in {execution_time:.3f}s, {rows_affected} rows, {index_usage}")
    ...
    def vacuum_database(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"VACUUM completed in {execution_time:.2f}s, reclaimed {space_reclaimed_mb:.2f}MB")
            return result
    ...
    def analyze_database(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if total_rows > 10000 and total_indexes < 5:
                recommendations.append("Database has many rows but few indexes - consider adding more indexes")
            
            if analysis['overall_stats']['database_size_mb'] > 100:
                recommendations.append("Large database detected - consider regular VACUUM operations")
            
            if len(self._performance_stats) > 100:
                avg_query_time = sum(s.execution_time for s in self._performance_stats) / len(self._performance_stats)
                if avg_query_time > 0.1:
                    recommendations.append(f"Average query time is high ({avg_query_time:.3f}s) - review slow queries")
            
            analysis['recommendations'] = recommendations
            
            logger.info(f"Database analysis completed: {len(tables)} tables, {total_rows} total rows")
    ...
    def optimize_for_large_datasets(self) -> Dict[str, Any]:
        ...
        try:
            ...
            optimizations['indexes_created'] = sum(len(indexes) for indexes in index_results.values())
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        if avg_time > 0.1:
            report['recommendations'].append("Average query time is high - review query optimization")
        
        if table_scan_count / total_queries > 0.5:
            report['recommendations'].append("Many queries use table scans - consider adding indexes")
        
        if slow_query_count > 0:
            report['recommendations'].append(f"Found {slow_query_count} slow queries - review and optimize")
