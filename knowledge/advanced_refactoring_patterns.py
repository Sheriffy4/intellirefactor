    ...
    def matches_class(self, class_node: ast.ClassDef, class_size: int) -> bool:
        ...
        if self.method_patterns:
            ...
            for pattern in self.method_patterns:
                pattern_matches += sum(1 for method in methods if re.match(pattern, method))
            
            min_matches = self.size_thresholds.get('min_pattern_matches', 1) if self.size_thresholds else 1
...
class AdvancedRefactoringPatterns:
    ...
    def get_applicable_patterns(
        ...
        for pattern in cls.get_all_patterns():
            if pattern.confidence >= min_confidence and pattern.matches_class(class_node, class_size):
