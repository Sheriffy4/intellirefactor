...
class AutomationMetadata:
    ...
    def generate_metadata(self, project_id: str, project_name: str) -> RefactoringAutomationMetadata:
...
''',
            ...
            container_configuration={
                "singleton_services": ["IDataService", "IConfigService", "ILoggingService"],
...
''',
    ...
    def get_automation_recommendations(self, 
                                     target_metrics: CodeMetrics, 
                                     target_context: RefactoringContext) -> List[str]:
    ...
    def _rule_applies(self, rule: CodeTransformationRule, 
                     metrics: CodeMetrics, context: RefactoringContext) -> bool:
        """Check if a transformation rule applies to the target code."""
        if rule.type == TransformationRuleType.EXTRACT_METHOD_TO_CLASS:
            return (metrics.file_size > 500 and 
                   metrics.number_of_responsibilities > 2 and
                   metrics.cohesion_level < 0.6)
        elif rule.type == TransformationRuleType.CREATE_FACADE:
            return (context.backward_compatibility_required and 
                   context.has_existing_clients)
        elif rule.type == TransformationRuleType.SPLIT_CONFIGURATION:
            return metrics.number_of_responsibilities > 3
            
        return False
        
    def _pattern_applies(self, pattern: DependencyInjectionPattern,
                        metrics: CodeMetrics, context: RefactoringContext) -> bool:
        """Check if a DI pattern applies to the target code."""
        if pattern.type == DIPatternType.CONSTRUCTOR_INJECTION:
            return (metrics.number_of_dependencies > 2 and
                   context.testing_infrastructure in ["good", "excellent"])
        return False
...
def generate_and_export_metadata(export_path: str = "automation_metadata.json",
    ...
    metadata = metadata_generator.generate_metadata("intellirefactor", "IntelliRefactor Project")
