...
class ExampleRefactoringPlugin(RefactoringPlugin):
    ...
    def metadata(self) -> PluginMetadata:
        return PluginMetadata(
            ...
            dependencies=[],
            config_schema={
                "min_string_length": {"type": "integer", "default": 10, "description": "Minimum string length for constant extraction"},
                "enable_fstring_conversion": {"type": "boolean", "default": True, "description": "Enable f-string conversion"},
                "enable_boolean_simplification": {"type": "boolean", "default": True, "description": "Enable boolean expression simplification"},
                "min_duplicate_lines": {"type": "integer", "default": 3, "description": "Minimum lines for duplicate code detection"}
            }
        )
    ...
    def _post_refactoring_hook(self, opportunity: Dict[str, Any], 
    ...
    def _extract_constants(self, file_path: str, ast_tree: ast.AST, 
        ...
        return {'constants_to_extract': constants_to_extract, 'transformations': transformations}
    
    def _convert_to_fstrings(self, file_path: str, ast_tree: ast.AST, 
                            context: Dict[str, Any]) -> Dict[str, Any]:
            ...
            def visit_BinOp(self, node):
                ...
                if isinstance(node.op, ast.Mod) and self._is_string_format(node):
                    conversions.append({
                        ...
                        'line': node.lineno,
                        'column': node.col_offset,
                        'original': ast.unparse(node) if hasattr(ast, 'unparse') else '<complex expression>',
    ...
    def _simplify_booleans(self, file_path: str, ast_tree: ast.AST, 
                          context: Dict[str, Any]) -> Dict[str, Any]:
                ...
                if len(node.ops) == 1 and len(node.comparators) == 1:
                    ...
                    if (isinstance(op, ast.Eq) and 
                        ...
                        simplifications.append({
                            ...
                            'line': node.lineno,
                            'column': node.col_offset,
                            'original': ast.unparse(node) if hasattr(ast, 'unparse') else '<comparison>',
                            'suggested': ast.unparse(left) if hasattr(ast, 'unparse') else '<simplified>'
                        })
            ...
            def visit_UnaryOp(self, node):
                ...
                if (isinstance(node.op, ast.Not) and 
                    ...
                    simplifications.append({
                        ...
                        'line': node.lineno,
                        'column': node.col_offset,
                        'original': ast.unparse(node) if hasattr(ast, 'unparse') else '<double negation>',
                        'suggested': ast.unparse(node.operand.operand) if hasattr(ast, 'unparse') else '<simplified>'
                    })
        ...
        for file_path, file_results in analysis_results.get('files', {}).items():
            if file_path.endswith('.py'):
                try:
                    ...
                    constant_results = self.hook_system.execute_custom_hooks('extract_constants', file_path, ast_tree, context)
                    fstring_results = self.hook_system.execute_custom_hooks('convert_fstrings', file_path, ast_tree, context)
                    boolean_results = self.hook_system.execute_custom_hooks('simplify_booleans', file_path, ast_tree, context)
                    
                    # Convert results to opportunities
                    for result_list in [constant_results, fstring_results, boolean_results]:
                            ...
                            if result and 'transformations' in result:
                                for transformation in result['transformations']:
                                    opportunities.append({
                                        ...
                                        'estimated_impact': self._estimate_impact(transformation)
                                    })
        ...
        if transformation_type == 'extract_constants':
            ...
            for constant in constants:
                if constant['type'] == 'numeric_constant':
                    constant_definitions.append(f"{constant['suggested_name']} = {constant['value']}")
                elif constant['type'] == 'string_constant':
                    constant_definitions.append(f"{constant['suggested_name']} = {repr(constant['value'])}")
