...
class ExampleKnowledgePlugin(KnowledgePlugin):
    ...
    def metadata(self) -> PluginMetadata:
        return PluginMetadata(
            ...
            dependencies=[],
            config_schema={
                "knowledge_db_path": {"type": "string", "default": "knowledge.json", "description": "Path to knowledge database"},
                "min_confidence": {"type": "number", "default": 0.7, "description": "Minimum confidence for recommendations"},
                "max_knowledge_items": {"type": "integer", "default": 1000, "description": "Maximum knowledge items to store"},
                "enable_pattern_learning": {"type": "boolean", "default": True, "description": "Enable automatic pattern learning"}
            }
        )
    ...
    def _pre_knowledge_extraction(self, analysis_results: Dict[str, Any], 
                                 refactoring_results: List[Dict[str, Any]]) -> None:
        """Prepare knowledge extraction context."""
        self.logger.debug("Pre-knowledge extraction hook")
        
        # Set up extraction context
        self.hook_system.set_context('knowledge_extraction', {
    ...
    def _post_knowledge_extraction(self, analysis_results: Dict[str, Any], 
        ...
        extraction_time = (datetime.now() - context.get('start_time', datetime.now())).total_seconds()
    ...
    def _learn_from_refactoring(self, opportunity: Dict[str, Any], 
        ...
        if len(self.knowledge_db['refactoring_history']) > max_items:
            self.knowledge_db['refactoring_history'] = self.knowledge_db['refactoring_history'][-max_items:]
        ...
        for file_path, file_results in analysis_results.get('files', {}).items():
            ...
            try:
                ...
                for cls in classes:
                    ...
                    if class_name.endswith(('Manager', 'Handler', 'Service', 'Controller', 'Factory')):
                        suffix = class_name[-7:] if class_name.endswith('Controller') else class_name[-7:]
                        for common_suffix in ['Manager', 'Handler', 'Service', 'Controller', 'Factory']:
                            if class_name.endswith(common_suffix):
                                suffix = common_suffix
                                break
                        pattern['common_suffixes'][suffix] = pattern['common_suffixes'].get(suffix, 0) + 1
    ...
    def _generate_recommendations(self, project_path: str, 
        ...
        for pattern_type, pattern_data in patterns.items():
            if isinstance(pattern_data, dict) and pattern_data.get('confidence', 0) >= min_confidence:
                recommendations['pattern_suggestions'].append({
                    'pattern': pattern_type,
                    'confidence': pattern_data['confidence'],
                    'suggestion': pattern_data.get('suggestion', f"Consider applying {pattern_type} pattern"),
    ...
    def extract_knowledge(self, analysis_results: Dict[str, Any], 
        ...
        self.hook_system.execute_hooks(HookType.PRE_KNOWLEDGE_EXTRACTION, analysis_results, refactoring_results)
        
        knowledge_items = []
        
        # Extract code patterns
        pattern_results = self.hook_system.execute_custom_hooks('extract_code_patterns', analysis_results)
        ...
        self.hook_system.execute_hooks(HookType.POST_KNOWLEDGE_EXTRACTION, 
                                     analysis_results, refactoring_results, knowledge_items)
