...
class FileAnalyzer:
    ...
    def _analyze_class(self, class_node: ast.ClassDef, lines: List[str]) -> ClassInfo:
        ...
        is_god_object = (
            len(methods) > 15
            or responsibilities_count > 5
            or sum(m.complexity for m in methods) > 100
        )
    ...
    def _analyze_method(self, method_node: ast.FunctionDef, lines: List[str]) -> MethodInfo:
        ...
        calls_count = len([node for node in ast.walk(method_node) if isinstance(node, ast.Call)])
    ...
    def _identify_issues(
        self, lines_count: int, classes: List[ClassInfo], complexity_score: int
    ) -> List[str]:
        ...
        if god_objects:
            for god_obj in god_objects:
                issues.append(
                    f"🔴 God Object: класс '{god_obj.name}' имеет {god_obj.public_methods_count} публичных методов и {god_obj.responsibilities_count} ответственностей"
                )
        ...
        if complex_methods:
            issues.append(
                f"🟡 Сложные методы ({len(complex_methods)}): {', '.join(complex_methods[:3])}{'...' if len(complex_methods) > 3 else ''}"
            )
    ...
    def _generate_recommendations(self, classes: List[ClassInfo], issues: List[str]) -> List[str]:
        ...
        if god_objects:
            recommendations.append(
                "🎯 Применить паттерн 'Extract Method to Component Class' для разделения God Objects"
            )
            recommendations.append("🏗️ Создать интерфейсы для каждой группы ответственностей")
            recommendations.append(
                "💉 Использовать Constructor Injection для связывания компонентов"
            )
        ...
        if god_objects or any("сложность" in issue.lower() for issue in issues):
            ...
            recommendations.append(
                "🔬 Применить property-based тестирование для проверки инвариантов"
            )
        ...
        if automation_potential > 0.7:
            recommendations.append(
                f"🤖 Высокий потенциал автоматизации ({automation_potential:.1%}) - можно использовать auto_refactor.py"
            )
        elif automation_potential > 0.4:
            recommendations.append(
                f"⚙️ Средний потенциал автоматизации ({automation_potential:.1%}) - частичная автоматизация возможна"
            )

        return recommendations

    def _calculate_priority(
        self, lines_count: int, classes: List[ClassInfo], issues_count: int
    ) -> int:
...
def main():
    ...
    if result.refactoring_priority > 6:
        ...
        print("   Рекомендуется немедленное вмешательство")
