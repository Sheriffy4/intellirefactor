...
class ProjectAnalyzer:
    ...
    def _load_knowledge_base(self):
        ...
        try:
            ...
            knowledge_file = (
                self.project_root / "knowledge" / "refactoring_automation_metadata.json"
            )
        ...
        try:
            ...
            for cls in classes:
                ...
                if len(class_methods) > 15:
                    issues.append(
                        f"God Object: класс {cls.name} имеет {len(class_methods)} методов"
                    )
    ...
    def analyze_project(
        ...
        refactoring_candidates.sort(key=lambda x: len(x.issues) * x.lines_count, reverse=True)
...
def main():
    ...
    parser.add_argument(
        "--project", "-p", default=".", help="Путь к проекту (по умолчанию: текущая папка)"
    )
    parser.add_argument("--output", "-o", help="Файл для сохранения отчета")
    parser.add_argument(
        "--include", nargs="*", default=["**/*.py"], help="Паттерны файлов для включения"
    )
    parser.add_argument(
        "--exclude",
        nargs="*",
    ...
    parser.add_argument(
        "--top", "-t", type=int, default=5, help="Количество топ кандидатов для показа"
    )
    ...
    if analysis.refactoring_candidates:
        ...
        for i, candidate in enumerate(analysis.refactoring_candidates[: args.top], 1):
            print(
                f"   {i}. {candidate.filepath} ({candidate.lines_count} строк, {len(candidate.issues)} проблем)"
            )
