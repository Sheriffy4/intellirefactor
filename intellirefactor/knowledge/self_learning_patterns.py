...
class SelfLearningSystem:
    ...
    def analyze_manual_corrections(
        ...
        logger.info(
            f"Analyzing manual corrections: {original_file} -> {len(corrected_files)} files"
        )
        ...
        import_patterns = self._extract_import_fix_patterns(
            original_content, corrected_contents
        )
        new_patterns.extend(import_patterns)

        # 2. Анализируем исправления логирования
        logging_patterns = self._extract_logging_fix_patterns(
            original_content, corrected_contents
        )
    ...
    def _extract_import_fix_patterns(
        self, original: str, corrected: Dict[str, str]
    ) -> List[LearningPattern]:
...
try:
    ...
    # Add fallback classes/functions here""",
            ...
            removed_imports = re.findall(
                r"# Removed non-existent import: (.+)", corrected_content
            )
            for removed_import in removed_imports:
                pattern = LearningPattern(
                    name="remove_nonexistent_import",
    ...
    def _extract_logging_fix_patterns(
        ...
        for file_path, corrected_content in corrected.items():
            corrected_log_vars = re.findall(
                r"(LOG|logger) = logging\.getLogger", corrected_content
            )
    ...
    def _extract_structural_patterns(
        ...
        for file_path, corrected_content in corrected.items():
            try:
                ...
                logger.warning(
                    f"Failed to parse corrected file {file_path} for structural analysis"
                )
                continue
    ...
    def _update_codebase_standards(
        ...
        logger_count = sum(
            content.count("logger = logging.getLogger")
            for content in corrected.values()
        )
        log_count = sum(
            content.count("LOG = logging.getLogger") for content in corrected.values()
        )
        ...
        for content in corrected.values():
            removed_imports = re.findall(
                r"# Removed non-existent import: from ([^\s]+)", content
            )
        ...
        for pattern in sorted(self.patterns, key=lambda p: p.confidence, reverse=True):
            try:
                if pattern.pattern_type == "import_fix":
                    ...
                    new_content = self._apply_structural_pattern(
            ...
            try:
                ...
                if standards_data:
                    self.codebase_standards = CodebaseStandards(
                        logging_variable=standards_data.get(
                            "logging_variable", "logger"
                        ),
                        import_style=standards_data.get("import_style", "absolute"),
                        error_handling_pattern=standards_data.get(
                            "error_handling_pattern", "try_except_fallback"
                        ),
                        preferred_modules=set(
                            standards_data.get("preferred_modules", [])
                        ),
                        deprecated_modules=set(
                            standards_data.get("deprecated_modules", [])
                        ),
    ...
    def get_pattern_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_confidence": sum(p.confidence for p in self.patterns)
            / len(self.patterns)
            if self.patterns
            else 0,
            "most_used_patterns": sorted(
                self.patterns, key=lambda p: p.usage_count, reverse=True
            )[:5],
            "learning_sessions": len(self.learning_history),
            "codebase_standards": {
                "logging_variable": self.codebase_standards.logging_variable,
                "deprecated_modules_count": len(
                    self.codebase_standards.deprecated_modules
                ),
            },
        }
