...
""",
    ...
    def _generate_decision_criteria(self) -> Dict[str, Any]:
        ...
        return {
            "extract_component": {
                "file_size_threshold": self.project_config.get(
                    "file_size_threshold", 500
                ),
                "complexity_threshold": self.project_config.get(
                    "complexity_threshold", 10
                ),
                "responsibility_count_threshold": self.project_config.get(
...
def main():
    ...
    try:
        ...
        metadata = generator.generate_metadata(
            "intellirefactor-project", "IntelliRefactor Project"
        )
        ...
        logger.info(
            f"  - {len(metadata['dependency_injection_patterns'])} dependency injection patterns"
        )
        logger.info(
            f"  - {len(metadata['interface_extraction_templates'])} interface extraction templates"
        )
        logger.info(
            f"  - {len(metadata['testing_strategies'])} testing strategy templates"
        )
        logger.info(
        ...
        for pattern in metadata["automation_recommendations"][
            "high_confidence_automatable"
        ]:
