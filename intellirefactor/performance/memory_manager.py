...
class BoundedMemoryProcessor(Generic[T]):
    ...
    def _process_batch(self, batch: List[T], processor: Callable[[List[T]], Any]) -> Any:
        ...
        try:
            ...
            logger.debug(
                f"Batch {self._batch_count} completed in {processing_time:.2f}s, "
    ...
    def _check_memory_and_cleanup(self):
        ...
        if current_memory > self.max_memory_mb:
            logger.warning(
                f"Memory usage ({current_memory:.1f}MB) exceeds limit ({self.max_memory_mb:.1f}MB)"
            )
    ...
    def get_processing_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_batch_size": (
                self._processed_count / self._batch_count if self._batch_count > 0 else 0
            ),
...
class MemoryManager:
    ...
    def create_bounded_processor(
        self, max_memory_mb: Optional[float] = None, batch_size: int = 100
    ...
    def optimize_for_large_project(self, project_size_estimate: int) -> Dict[str, Any]:
        ...
        logger.info(
            f"Optimized memory settings for project size {project_size_estimate}: {settings}"
        )
        return settings
    ...
    def get_memory_report(self) -> Dict[str, Any]:
        ...
        if len(history) >= 2:
            ...
            older_avg = (
                sum(s.rss_mb for s in history[-10:-5]) / min(5, len(history) - 5)
                if len(history) > 5
                else recent_avg
            )
            trend = (
