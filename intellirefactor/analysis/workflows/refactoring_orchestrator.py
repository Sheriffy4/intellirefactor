...
class RefactoringOrchestrator:
    ...
    def create_refactoring_plan(
        self, target_file: Path, refactoring_opportunities: List[Dict[str, Any]]
    ) -> RefactoringPlan:
        """
        ...
        steps.append(
            RefactoringStep(
                ...
                description="Validate original file syntax",
                target_file=target_file,
            ...
            for i, step in enumerate(plan.steps):
                ...
                if not step_result["success"]:
                    ...
                    if plan.rollback_available:
                        ...
                        if rollback_result["success"]:
                            ...
                            results["errors"].append(
                                f"Rollback failed: {rollback_result.get('error', 'Unknown error')}"
                            )
                ...
                if self.validation_enabled and step.operation_type != "validate_syntax":
                    ...
                    if not validation_result["success"]:
                        results["errors"].append(
                            f"Step {i + 1} validation failed: {validation_result.get('error', 'Unknown error')}"
                        )
            ...
            if plan.rollback_available:
                try:
                    ...
                    if rollback_result["success"]:
                        ...
                        results["errors"].append(
                            f"Emergency rollback failed: {rollback_result.get('error', 'Unknown error')}"
                        )
