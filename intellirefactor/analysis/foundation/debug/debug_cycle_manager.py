...
from intellirefactor.analysis.foundation.models import AnalysisError, AnalysisStage, Severity, parse_severity
from intellirefactor.analysis.foundation.error_handler import StandardErrorHandler

logger = logging.getLogger(__name__)

#
# Backward compatible name:
# old code may import ErrorSeverity from this module.
#
...
class Issue:
    ...
    error_details: Optional[AnalysisError] = None  # foundation.models.AnalysisError
...
class DebugCycleManager:
    ...
    def detect_intellirefactor_issues(
        ...
        try:
            ...
            if additional_info and isinstance(additional_info, dict):
                file_path = additional_info.get("file_path")

            # Foundation AnalysisError (stable contract)
            handled_error = AnalysisError(
                ...
                metadata={"context": context, **(additional_info or {})},
            )
            # keep in handler collection (optional, but consistent)
            self.error_handler.add_error(handled_error)

            # Convert to Issue objects
            issues = []

            # Primary issue from the error
            primary_issue = self._create_issue_from_error(handled_error, context, original_exc=error)
            issues.append(primary_issue)

            # Detect secondary issues based on patterns
            secondary_issues = self._detect_secondary_issues(
                handled_error, context, additional_info
            )
    ...
    def execute_debug_cycle(
        ...
        cycle = DebugCycle(cycle_id=cycle_id, start_time=self._get_timestamp())
        ...
        try:
            ...
            logger.info(
                f"Debug cycle {cycle_id} completed. Success: {cycle.success}, Fixes: {successful_fixes}/{len(issues)}"
            )
            ...
            cycle.validation_results.append(
                {"cycle_error": str(cycle_error), "timestamp": self._get_timestamp()}
            )
    ...
    def create_checkpoint(self, state_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            checkpoint = {
                ...
                "debug_progress": (
                    self.current_progress.to_dict() if self.current_progress else None
                ),
    ...
    def _create_issue_from_error(
        ...
        *,
        ...
        """Create an Issue from a foundation AnalysisError + original exception."""
        msg = (handled_error.message or "").lower()
        ctx = (context or "").lower()

        issue_type = self._categorize_issue_type(msg, ctx)
        severity = self._determine_severity(original_exc, msg, ctx)
        suggested = self._suggest_fixes(issue_type, msg, ctx)
        ...
        return Issue(
            ...
            diagnostic_info=dict(handled_error.metadata or {}),
            timestamp=self._get_timestamp(),
        )

    def _categorize_issue_type(self, msg_lower: str, ctx_lower: str) -> IssueType:
        text = f"{ctx_lower} {msg_lower}"
        for pattern, itype in self.issue_patterns.items():
            if pattern in text:
                return itype
        # heuristic fallback
        if "syntax" in text or "parse" in text:
            return IssueType.ANALYSIS_FAILURE
        if "import" in text or "module" in text or "package" in text:
            return IssueType.DEPENDENCY_ISSUE
        if "test" in text or "validate" in text:
            return IssueType.VALIDATION_FAILURE
        if "timeout" in text or "memory" in text:
            return IssueType.PERFORMANCE_ISSUE
        return IssueType.UNKNOWN_ISSUE

    def _determine_severity(
        self, exc: Optional[Exception], msg_lower: str, ctx_lower: str
    ) -> Severity:
        ...
        if isinstance(exc, (SyntaxError, FileNotFoundError)):
            return Severity.MEDIUM
        if isinstance(exc, TimeoutError) or "timeout" in msg_lower:
            return Severity.LOW
        if "config" in ctx_lower or "configuration" in ctx_lower:
            return Severity.HIGH
        return Severity.MEDIUM

    def _suggest_fixes(self, issue_type: IssueType, msg_lower: str, ctx_lower: str) -> List[str]:
        fixes: List[str] = []
        if issue_type == IssueType.CONFIGURATION_ISSUE:
            fixes += ["Check IntelliRefactor configuration", "Try default config"]
        elif issue_type == IssueType.DEPENDENCY_ISSUE:
            fixes += ["Install missing dependencies", "Verify environment/venv is active"]
        elif issue_type == IssueType.ANALYSIS_FAILURE:
            fixes += ["Check file readability and syntax", "Run with --verbose to see stack trace"]
        elif issue_type == IssueType.VALIDATION_FAILURE:
            fixes += ["Run tests / create characterization tests", "Relax validation or add excludes"]
        elif issue_type == IssueType.PERFORMANCE_ISSUE:
            fixes += ["Reduce scope (include/exclude patterns)", "Use incremental/index-only modes"]
        else:
            fixes += ["Report issue with context and stack trace"]
    ...
    def _detect_secondary_issues(
        ...
        error_message = (handled_error.message or "").lower()
        ...
        if "config" in error_message:
            config_issue = Issue(
                ...
                suggested_fixes=[
                    "Check IntelliRefactor configuration",
        ...
        if (
            any(keyword in error_message for keyword in ["import", "module", "package"])
        ):
            dep_issue = Issue(
                issue_id=self._generate_issue_id(),
    ...
    def _determine_fix_strategy(self, issue: Issue) -> Dict[str, Any]:
        ...
        base_strategy = self.fix_strategies.get(
            issue.issue_type, self.fix_strategies[IssueType.UNKNOWN_ISSUE]
        )
    ...
    def _apply_configuration_fix(
        self, issue: Issue, strategy: Dict[str, Any]
    ...
    def _apply_dependency_fix(
        ...
        rollback_info = {
            "type": "dependency",
            "reversible": False,
        }  # Package installs are not easily reversible
