...
class CallerAnalyzer:
    ...
    def find_external_callers(self, target_module: str) -> List[ExternalCaller]:
        ...
        """
        logger.info(f"Finding external callers for module candidates: {self.module_candidates}")
        ...
        logger.info(f"Found {len(external_callers)} external callers")
    ...
    def _resolve_relative_import(self, py_file: Path, module_name: str, level: int) -> Optional[str]:
        ...
        try:
            # Get the importing file's module path
            rel_path = py_file.relative_to(self.project_root)
            file_module = str(rel_path).replace('\\', '.').replace('/', '.')
            if file_module.endswith('.py'):
                file_module = file_module[:-3]
            
            # Split into parts and go up 'level' directories
            parts = file_module.split('.')
            if level > 0:
                parts = parts[:-level] if level < len(parts) else []
            
            # Add the relative module name
            if module_name:
