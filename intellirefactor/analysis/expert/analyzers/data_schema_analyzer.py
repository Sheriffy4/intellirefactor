...
Analyzes data structures like params and packet_info to understand
what keys are actually used and where, creating a practical schema.
"""
...
class DataSchemaAnalyzer:
    ...
    def analyze_data_schemas(self, module_ast: ast.Module) -> Dict[str, any]:
        ...
        return {
            "key_usage_map": key_usage,
            "parameter_schemas": parameter_schemas,
            "data_flow_patterns": data_flow,
            "schema_recommendations": schema_recommendations,
            "refactoring_risks": self._assess_schema_refactoring_risks(key_usage, parameter_schemas)
        }
    ...
    def _analyze_dictionary_keys(self, module_ast: ast.Module) -> Dict[str, Dict[str, any]]:
        ...
        for node in ast.walk(module_ast):
            ...
            if isinstance(node, ast.Subscript):
                # dict["key"] pattern
                key_info = self._extract_subscript_key(node)
                if key_info:
                    self._record_key_usage(key_usage, key_info, "subscript", node)
            
            elif isinstance(node, ast.Call):
                # dict.get("key") pattern
                if isinstance(node.func, ast.Attribute) and node.func.attr == 'get':
                    key_info = self._extract_get_key(node)
                    if key_info:
                        self._record_key_usage(key_usage, key_info, "get", node)
                
                # "key" in dict pattern (handled in ast.Compare)
            
            elif isinstance(node, ast.Compare):
                # "key" in dict pattern
                key_info = self._extract_in_key(node)
                if key_info:
                    self._record_key_usage(key_usage, key_info, "in_check", node)
    ...
    def _record_key_usage(self, key_usage: Dict, key_info: Dict, access_type: str, node: ast.AST):
    ...
    def _find_containing_method(self, node: ast.AST) -> str:
