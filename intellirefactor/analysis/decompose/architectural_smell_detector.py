...
def _make_evidence(
    *,
    ...
    return Evidence(
        ...
        file_references=[FileReference(file_path=file_path, line_start=int(line_start), line_end=int(line_end))],
...
class ArchitecturalSmell:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "extraction_candidates": list(self.extraction_candidates),
            "responsibility_markers": [m.to_dict() for m in self.responsibility_markers],
        }
...
class ArchitecturalSmellDetector:
    ...
    def _detect_god_class(
        ...
        is_god_class = (
            method_count > self.thresholds.god_class_methods
            or responsibility_count > self.thresholds.god_class_responsibilities
            or cohesion < self.thresholds.god_class_cohesion
            or lines > self.thresholds.god_class_lines
        )
        ...
        severity_score = 0
        if method_count > self.thresholds.god_class_methods * 2:
            severity_score += 2
        elif method_count > self.thresholds.god_class_methods:
            severity_score += 1

        if responsibility_count > self.thresholds.god_class_responsibilities * 2:
            severity_score += 2
        elif responsibility_count > self.thresholds.god_class_responsibilities:
            severity_score += 1

        if cohesion < self.thresholds.god_class_cohesion / 2:
            severity_score += 2
        elif cohesion < self.thresholds.god_class_cohesion:
            severity_score += 1
    ...
    def _detect_long_method(
        ...
        is_long_method = (
            lines > self.thresholds.long_method_lines
            or statements > self.thresholds.long_method_statements
        )
        ...
        severity_score = 0
        if lines > self.thresholds.long_method_lines * 2:
            severity_score += 2
        elif lines > self.thresholds.long_method_lines:
            severity_score += 1

        if statements > self.thresholds.long_method_statements * 2:
            severity_score += 2
        elif statements > self.thresholds.long_method_statements:
            severity_score += 1
        ...
        evidence = _make_evidence(
            ...
            metadata={"lines": lines, "statements": statements},
        )
    ...
    def _detect_high_complexity(
        ...
        is_high_complexity = (
            complexity > self.thresholds.high_complexity_cyclomatic
            or nesting_depth > self.thresholds.high_complexity_nesting
        )
        ...
        severity_score = 0
        if complexity > self.thresholds.high_complexity_cyclomatic * 2:
            severity_score += 2
        elif complexity > self.thresholds.high_complexity_cyclomatic:
            severity_score += 1

        if nesting_depth > self.thresholds.high_complexity_nesting * 1.5:
            severity_score += 2
        elif nesting_depth > self.thresholds.high_complexity_nesting:
            severity_score += 1
        ...
        recommendations = ["Reduce conditional complexity", "Apply Guard Clauses"]
    ...
    def _detect_srp_violation(
        ...
        is_srp_violation = (
            responsibility_count > self.thresholds.srp_responsibility_keywords
            or external_deps > self.thresholds.srp_external_dependencies
        )
    ...
    def _detect_inappropriate_intimacy(
        ...
        coupling_ratio = (
            private_accesses / total_external_attr_accesses
            if total_external_attr_accesses > 0
            else 0
        )

        if (
            coupling_ratio <= self.thresholds.intimacy_coupling_ratio
            and private_accesses < self.thresholds.intimacy_private_access * 2
        ):
            return None
    ...
    def _calculate_cyclomatic_complexity(
        self, method_node: Union[ast.FunctionDef, ast.AsyncFunctionDef]
    ) -> int:
        complexity = 1
    ...
    def _generate_decomposition_strategy(self, responsibilities: List[ResponsibilityMarker]) -> str:
        ...
        for resp_type, resp_list in responsibility_groups.items():
            if len(resp_list) > 1:
                strategies.append(
                    f"Extract {resp_type} operations into a separate {resp_type.title()}Handler class"
                )
