...
class BlockCloneDetector:
    ...
    def _group_by_fingerprint(
        self, blocks: List[BlockInfo], fingerprint_attr: str
    ) -> Dict[str, List[BlockInfo]]:
    ...
    def _create_evidence(
        ...
        description = (
            f"{clone_type.value.title()} clone detected with {len(group_blocks)} instances"
        )
    ...
    def _create_group_evidence(
        self, instances: List[CloneInstance], clone_type: CloneType
    ) -> Evidence:
    ...
    def _detect_hybrid_clones(
        self, blocks: List[BlockInfo], covered_blocks: Set[BlockKey]
    ) -> List[CloneGroup]:
        """
        ...
        for pair_map, channels in pair_configs:
            for group_blocks in pair_map.values():
                ...
                clone_group = self._create_clone_group(uncovered, CloneType.HYBRID, channels)
