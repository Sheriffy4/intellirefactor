...
class IndexBuilder:
    ...
    def build_index(
        ...
        try:
            ...
            for batch_idx, batch in enumerate(self._batch_files(files_to_process)):
                ...
                self.logger.info(
                    f"Processed batch {batch_idx + 1}: {batch_result['files']} files, "
            ...
            self.logger.info(
                f"Index build completed in {build_time:.2f}s: "
    ...
    def rebuild_index(
        ...
        return self.build_index(
            project_path, incremental=False, progress_callback=progress_callback
        )
    ...
    def _filter_changed_files(self, python_files: List[Path], project_root: Path) -> List[Path]:
        ...
        self.logger.info(
            f"Incremental update: {len(changed_files)} changed files out of {len(python_files)} total"
        )
