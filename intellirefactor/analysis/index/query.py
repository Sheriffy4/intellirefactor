...
class IndexQuery:
            ...
            for row in cursor.fetchall():
                ...
                for block_id in block_ids:
                    block_cursor = conn.execute(
                        """
                        SELECT b.block_uid, b.block_type, b.line_start, b.line_end,
        ...
        def traverse(current_uid: str, depth: int):
            ...
            with self.store._get_connection() as conn:
                cursor = conn.execute(
                    """
                    SELECT
                        COALESCE(t.symbol_uid, d.target_external) AS target_key,
                        d.dependency_kind,
                        d.confidence,
                        COALESCE(d.usage_count, 1) AS usage_count,
                        CASE WHEN t.symbol_uid IS NULL THEN 0 ELSE 1 END AS is_internal
                    FROM dependencies d
                    JOIN symbols s ON d.source_symbol_id = s.symbol_id
                    LEFT JOIN symbols t ON d.target_symbol_id = t.symbol_id
                    WHERE s.symbol_uid = ?
                    """,
                    (current_uid,),
                )
                ...
                for row in cursor.fetchall():
                    target, kind, confidence, usage_count, is_internal = row
                    graph["edges"].append(
                        {
                            "source": current_uid,
                            "target": target,
                            ...
                            "usage_count": int(usage_count or 1),
                    ...
                    if is_internal and isinstance(target, str) and target:
    ...
    def find_importers_of_module(self, module_path: str) -> List[Dict[str, Any]]:
        ...
        with self.store._get_connection() as conn:
            cursor = conn.execute(
                ...
                SELECT
                    f.file_path,
                    GROUP_CONCAT(DISTINCT s.symbol_uid) AS importer_symbol_uids,
                    GROUP_CONCAT(DISTINCT s.qualified_name) AS importer_qualified_names
                FROM dependencies d
                JOIN symbols s ON d.source_symbol_id = s.symbol_id
                JOIN files f ON s.file_id = f.file_id
                WHERE d.dependency_kind = 'imports'
                  AND (d.target_external = ? OR d.target_external LIKE ?)
            ...
            for row in cursor.fetchall():
                importers.append(
                    {
                        "importer_path": row[0],
                        "importer_symbol_uids": (row[1].split(",") if row[1] else []),
                        "importer_qualified_names": (row[2].split(",") if row[2] else []),
                    }
                )
    ...
    def find_usage_of_symbol(self, symbol_uid: str) -> List[Dict[str, Any]]:
        ...
        with self.store._get_connection() as conn:
            ...
            try:
                cur = conn.execute("SELECT symbol_id FROM symbols WHERE symbol_uid = ? LIMIT 1", (symbol_uid,))
                r = cur.fetchone()
                if r:
                    target_symbol_id = int(r[0])
            except Exception:
            ...
            cursor = conn.execute(
                ...
                JOIN files f ON s.file_id = f.file_id
                LEFT JOIN symbols t ON d.target_symbol_id = t.symbol_id
                WHERE (d.target_external = ? OR (? IS NOT NULL AND d.target_symbol_id = ?))
                """,
                (symbol_uid, target_symbol_id, target_symbol_id),
            )
                ...
                usages.append(
                    {
                        ...
                        "target": row[1],  # symbol_uid (internal) or external string
