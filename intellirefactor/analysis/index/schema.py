...
class IndexSchema:
    ...
    # v3: path normalization (posix), dependency usage_count aggregation improvements
    SCHEMA_VERSION = 3

    _REQUIRED_COLS = {
        "dependencies": {"dependency_kind", "usage_count"},
        "blocks": {"block_type", "normalized_fingerprint"},
        "symbols": {"is_static", "is_classmethod"},
    }
    ...
    INDEXES = [
        ...
        # Keep cyclomatic_complexity index for compatibility / future use,
        # but also index the field that is actively populated by current builder.
        "CREATE INDEX IF NOT EXISTS idx_symbols_complexity ON symbols(cyclomatic_complexity)",
        "CREATE INDEX IF NOT EXISTS idx_symbols_complexity_score ON symbols(complexity_score)",
    ...
    @classmethod
    def generate_block_uid(cls, symbol_uid: str, kind: str, line_start: int, line_end: int) -> str:
    ...
    def needs_migration(cls, conn: sqlite3.Connection) -> bool:
        ...
        if current_version is None or current_version != cls.SCHEMA_VERSION:
            return True
        return not cls._has_required_columns(conn)

    @classmethod
    def _has_required_columns(cls, conn: sqlite3.Connection) -> bool:
        """
        Defensive check: schema_info version can lie; validate required columns exist.
        """
        try:
            for table, required in cls._REQUIRED_COLS.items():
...
def create_symbol_record(
    ...
    *,
    ...
    # IMPORTANT:
    # symbol_uid stability depends on *real file_path*, not synthetic file_id.
    # For backward compatibility we allow missing file_path and fall back to file_id placeholder.
    file_key = file_path or f"file_{file_id}"
    symbol_uid = IndexSchema.generate_symbol_uid(
        file_key, qualified_name, kind, line_start, signature
    )
