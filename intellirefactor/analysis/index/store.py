...
class IndexStore:
    ...
    def __init__(self, db_path: Path):
        ...
        # If schema version is outdated, recreate DB (same policy as IndexBuilder).
        if not needs_init and self.db_path.exists():
            try:
                with sqlite3.connect(str(self.db_path)) as conn:
                    conn.execute("PRAGMA foreign_keys = ON")
                    if IndexSchema.needs_migration(conn):
                        needs_init = True
                        self.logger.warning(
                            "Index schema migration needed - recreating database at %s",
                            self.db_path,
                        )
            except Exception:
                needs_init = True
                self.logger.warning(
                    "Failed to check schema version - recreating database at %s",
                    self.db_path,
                    exc_info=True,
                )
        ...
        if needs_init:
            # IMPORTANT: create_database() uses CREATE TABLE IF NOT EXISTS.
            # For true migration we recreate file to avoid mixed schemas.
            try:
                if self.db_path.exists():
                    self.db_path.unlink()
            except Exception:
                self.logger.warning("Could not remove existing DB file %s", self.db_path, exc_info=True)
            conn = IndexSchema.create_database(str(self.db_path))
    ...
    def store_blocks(self, symbol_ids: Dict[str, int], blocks: List[Dict[str, Any]]) -> None:
        ...
        with self.transaction() as conn:
            for block in blocks:
                ...
                # Canonical schema uses `block_type` only (v3)
                ...
                conn.execute(
                    """
                    INSERT INTO blocks (
                        block_uid, symbol_id, block_type, line_start, line_end,
                        lines_of_code, statement_count, nesting_level,
    ...
    def get_blocks_by_symbol(self, symbol_id: int) -> List[Dict[str, Any]]:
        ...
        with self._get_connection() as conn:
            cursor = conn.execute(
                """
                SELECT block_id, block_uid, block_type, line_start, line_end,
                       lines_of_code, statement_count, nesting_level,
                       ast_fingerprint, token_fingerprint, normalized_fingerprint
    ...
    def store_dependencies(
        ...
        with self.transaction() as conn:
            for dep in dependencies:
                ...
                # Canonical schema only (v3)
                dep_kind = str(dep.get("dependency_kind") or "").strip()
                if not dep_kind:
                    dep_kind = "unknown"
                usage_count = int(dep.get("usage_count", 1) or 1)
                conn.execute(
                    """
                    INSERT INTO dependencies (
                        source_symbol_id, target_symbol_id, target_external,
                        dependency_kind, resolution, confidence, evidence_json, usage_count
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
                        source_symbol_id,
                ...
                dependencies.append(
                    {
                        ...
                        "dependency_kind": row[3],
                        "resolution": row[4],
                        "confidence": row[5],
                        "evidence": json.loads(row[6]) if row[6] else {},
                        "usage_count": int(row[7] or 1),
                    }
                )
    ...
    def bulk_insert_dependencies(self, dependencies_data: List[Tuple]) -> None:
        ...
        with self.transaction() as conn:
            ...
            first_len = len(dependencies_data[0])
            has_usage_count = first_len >= 7
            if has_usage_count:
                conn.executemany(
                    """
                    INSERT INTO dependencies (
                        source_symbol_id, target_external, dependency_kind, resolution,
                        confidence, evidence_json, usage_count
                    ) VALUES (?, ?, ?, ?, ?, ?, ?)
                    """,
                    [tuple(x[:7]) for x in dependencies_data],
                )
            else:
                conn.executemany(
                    """
                    ...
                    """,
                    [tuple(x[:6]) for x in dependencies_data],
                )
    ...
    def store_dependency_info(
        ...
        with self.transaction() as conn:
            ev = dependency_info.evidence
            if hasattr(ev, "to_dict"):
                ev_json = json.dumps(ev.to_dict())
            else:
                ev_json = json.dumps(ev or {})
            cursor = conn.execute(
                """
        ...
        with self._get_connection() as conn:
            cursor = conn.execute(
                ...
                SELECT s.qualified_name, s.name, s.kind, s.line_start, s.line_end,
                       ...
                       s.responsibility_markers, s.ast_fingerprint, s.token_fingerprint,
                       f.file_path
                FROM symbols s
                JOIN files f ON s.file_id = f.file_id
                WHERE s.kind = 'method'
            ...
            for row in cursor.fetchall():
                ...
                semantic_category = parse_semantic_category(semantic_category_str)
                markers_data: List[str] = []
                if responsibility_markers_json:
                    try:
                        raw = json.loads(responsibility_markers_json)
                        if isinstance(raw, list):
                            markers_data = [str(x) for x in raw if x]
                    except Exception:
                        markers_data = []
                responsibility_markers = parse_responsibility_markers(markers_data)
                ...
                method_info = DeepMethodInfo(
                    ...
                    token_fingerprint=token_fingerprint or "",
