...
class ResponsibilityClusterer:
    ...
    def cluster_class_methods(
        self, source_code: str, class_name: str, file_path: str
    ...
    def _calculate_method_similarity(self, method1: MethodInfo, method2: MethodInfo) -> float:
        ...
        attr_similarity = self._jaccard_similarity(method1.self_attributes, method2.self_attributes)

        # Dependency similarity
        dep_similarity = self._jaccard_similarity(method1.external_calls, method2.external_calls)
    ...
    def _perform_clustering(
        self, methods: List[MethodInfo], similarity_matrix: np.ndarray
    ) -> List[List[int]]:
    ...
    def _community_detection_clustering(
        self, methods: List[MethodInfo], similarity_matrix: np.ndarray
    ) -> List[List[int]]:
    ...
    def _evaluate_cluster_quality(
        self, clusters: List[List[int]], similarity_matrix: np.ndarray
    ) -> float:
    ...
    def _evaluate_clustering_quality(
        ...
        for i, cluster_indices in enumerate(clusters):
            ...
            cluster = self._create_responsibility_cluster(
                f"cluster_{i}", cluster_methods, similarity_matrix, cluster_indices
            )
        ...
        if responsibility_clusters:
            average_cohesion = sum(c.cohesion_score for c in responsibility_clusters) / len(
                responsibility_clusters
            )
        else:
        ...
        extraction_recommended = (
            len(responsibility_clusters) >= 2
            and unclustered_ratio <= self.config.max_unclustered_ratio
            and average_cohesion >= self.config.min_cohesion
            and silhouette >= self.config.min_silhouette_score
        )
    ...
    def _create_responsibility_cluster(
        self,
        cluster_id: str,
        methods: List[MethodInfo],
        similarity_matrix: np.ndarray,
        ...
        if methods:
            ...
            shared_attributes = {
                attr for attr, count in all_attributes.items() if count >= threshold
            }
            shared_dependencies = {
                dep for dep, count in all_dependencies.items() if count >= threshold
            }
        else:
        ...
        confidence = self._calculate_cluster_confidence(
            methods, cohesion_score, shared_attributes, shared_dependencies
        )
        ...
        # Generate evidence
        evidence = self._generate_cluster_evidence(
            cluster_id, methods, cohesion_score, shared_attributes, shared_dependencies
        )
    ...
    def _assess_cluster_quality(
        self, methods: List[MethodInfo], cohesion_score: float
    ) -> ClusterQuality:
        ...
        if (
            cluster_size <= self.config.max_cluster_size
            and cohesion_score >= self.config.min_cohesion
        ):
    ...
    def _generate_clustering_evidence(
        self,
        class_name: str,
        clusters: List[ResponsibilityCluster],
        unclustered_ratio: float,
        ...
        confidence = self._calculate_overall_confidence(
            clusters, unclustered_ratio, average_cohesion, silhouette_score
        )
    ...
    def _generate_cluster_evidence(
        self,
        cluster_id: str,
        methods: List[MethodInfo],
        cohesion_score: float,
            ...
            for cluster in clusters:
                if cluster.quality == ClusterQuality.HIGH:
                    recommendations.append(
                        f"Extract {cluster.suggested_name} with methods: {', '.join([m.name for m in cluster.methods])}"
                    )

        if unclustered_ratio > self.config.max_unclustered_ratio:
            recommendations.append(
                "Consider manual review of unclustered methods for additional extraction opportunities"
            )

        if not clusters:
            recommendations.append(
                "No clear clustering patterns found - consider alternative refactoring approaches"
            )

        recommendations.append(
            "Use composition or delegation to maintain loose coupling between extracted components"
        )

        return recommendations

    def _create_insufficient_result(
        self, class_name: str, file_path: str, methods: List[MethodInfo]
    ) -> ClusteringResult:
        ...
        return ClusteringResult(
            ...
            evidence=evidence,
            recommendations=[
                "Class is too small for responsibility clustering - consider other refactoring approaches"
            ],
        )
    ...
    def generate_component_suggestions(
        ...
        for cluster in clustering_result.clusters:
            if (
                cluster.quality == ClusterQuality.HIGH
                and cluster.confidence >= self.config.min_confidence
            ):
                # Generate interface definition
                interface = self._generate_component_interface(
                    cluster, clustering_result.class_name
                )

                # Generate extraction plan
                extraction_plan = self._generate_extraction_plan(
                    cluster, clustering_result.class_name, clustering_result.file_path
                )
    ...
    def _generate_component_interface(
        self, cluster: ResponsibilityCluster, source_class: str
    ) -> ComponentInterface:
        ...
        complexity = self._assess_extraction_complexity(
            cluster, required_attributes, external_dependencies
        )
        ...
        constructor_parameters = self._generate_constructor_parameters(
            required_attributes, external_dependencies
        )
    ...
    def _generate_extraction_plan(
        self, cluster: ResponsibilityCluster, source_class: str, file_path: str
    ) -> ExtractionPlan:
        ...
        complexity = self._assess_extraction_complexity(
            cluster, attributes_to_extract, dependencies_to_inject
        )
        estimated_effort = self._estimate_extraction_effort(cluster, complexity)

        # Identify risk factors
        risk_factors = self._identify_extraction_risks(
            cluster, attributes_to_extract, dependencies_to_inject
        )
    ...
    def _assess_extraction_complexity(
        self,
        cluster: ResponsibilityCluster,
        attributes: Set[str],
    ...
    def _determine_extraction_type(self, cluster: ResponsibilityCluster) -> str:
        ...
        dominant_resp = (
            cluster.dominant_responsibilities[0] if cluster.dominant_responsibilities else ""
        )
    ...
    def _estimate_extraction_effort(
        self, cluster: ResponsibilityCluster, complexity: ExtractionComplexity
    ) -> int:
    ...
    def _identify_extraction_risks(
        self,
        cluster: ResponsibilityCluster,
        attributes: Set[str],
        dependencies: Set[str],
    ...
    def _generate_implementation_steps(
        self, cluster: ResponsibilityCluster, extraction_type: str
    ) -> List[str]:
    ...
    def _generate_extraction_rationale(
        self, cluster: ResponsibilityCluster, interface: ComponentInterface
    ) -> str:
        """Generate rationale for extraction suggestion."""

        rationale = (
            f"Extract {interface.component_name} component with {len(cluster.methods)} methods "
        )
    ...
    def _generate_extraction_benefits(
        self, cluster: ResponsibilityCluster, interface: ComponentInterface
    ) -> List[str]:
    ...
    def _generate_extraction_trade_offs(
        self, cluster: ResponsibilityCluster, plan: ExtractionPlan
    ) -> List[str]:
        ...
        if plan.complexity in [
            ...
            ExtractionComplexity.VERY_HIGH,
        ]:
            trade_offs.append(
                f"High extraction complexity requires {plan.estimated_effort_hours} hours of effort"
            )
    ...
    def _generate_suggestion_evidence(
        self,
        cluster: ResponsibilityCluster,
        interface: ComponentInterface,
        plan: ExtractionPlan,
    ...
    def _generate_constructor_parameters(
        self, attributes: Set[str], dependencies: Set[str]
    ) -> List[str]:
    ...
    def _generate_component_docstring(
        self, cluster: ResponsibilityCluster, source_class: str
    ) -> str:
        ...
        docstring += (
            f"Handles {responsibilities} responsibilities with {len(cluster.methods)} methods.\n"
        )
