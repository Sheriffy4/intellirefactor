    ...
    def _resolve_call_enhanced(
        ...
        if "." in raw_call:
            ...
            if recv in lth:
                ...
                if cls_name in mcm:
                    if meth in mcm[cls_name]:
                        return mcm[cls_name][meth], "local_type_hint"
                    # Class found but method not → inherited from base class
                    return None, UnresolvedReason.baseclass_method.value

                # Try global search across all modules
                for m, classes in indices["module_class_method"].items():
                    if cls_name in classes:
                        if meth in classes[cls_name]:
                            return classes[cls_name][meth], "local_type_hint"
                        return None, UnresolvedReason.baseclass_method.value

                # Class in local file definitions → inherited method
                if cls_name in (class_definitions or set()):
                    return None, UnresolvedReason.baseclass_method.value

                # Class in module_class_defs for that module
                if cls_name in module_class_defs.get(mod, set()):
                    return None, UnresolvedReason.baseclass_method.value

                # Typed but class not found anywhere → assume inherited (non-actionable)
                return None, UnresolvedReason.baseclass_method.value
