...
class FunctionCategorizer:
    ...
    def _categorize_by_module(self, block: FunctionalBlock) -> Tuple[str, str]:
        ...
        # analysis subdomains
        if module.startswith("analysis.decomposition."):
            return "analysis", "decomposition"
        if module.startswith("analysis.expert."):
            return "analysis", "expert"
        if module.startswith("analysis.index_") or module.startswith("analysis.indexing."):
            return "analysis", "indexing"

        if module in {
            "analysis.debug_cycle_manager",
            "analysis.incremental_debugging_workflow",
            "analysis.lazy_loader",
            "analysis.refactoring_orchestrator",
        }:
            return "analysis", "workflow"
