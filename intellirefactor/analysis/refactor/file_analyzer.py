...
from intellirefactor.interfaces import BaseFileAnalyzer, GenericAnalysisResult
...
class FileAnalyzer(BaseFileAnalyzer):
    ...
    def _enrich_with_context(
        ...
        try:
            try:
                from intellirefactor.analysis.lazy_loader import LazyProjectContext as _LPC  # optional
            except Exception:
    ...
    def generate_detailed_report(self, result: GenericAnalysisResult) -> str:
        ...
        if nested_classes or nested_functions:
            ...
            for item in items[:limit]:
                kind = item.get("kind", "?")
                qn = item.get("qualname", item.get("name", "?"))
                ls, le = item.get("line_start", "?"), item.get("line_end", "?")

                if kind == "class":
                    cplx = item.get("total_complexity", 0)
                    params = "-"
                    calls = "-"
                else:
                    cplx = item.get("complexity", 0)
                    params = item.get("parameters_count", 0)
                    calls = item.get("calls_count", 0)
