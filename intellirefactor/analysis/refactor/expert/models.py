...
class CallGraph:
    ...
    def get_callees(self, method_name: str) -> List[str]:
        ...
        return [edge.callee for edge in self.edges if edge.caller == method_name]
...
def _jsonify(obj: Any) -> Any:
    """
    Stable JSON-safe serialization for expert dataclasses.
    - Enums -> .value
    - dataclasses -> dict of fields recursively
    - sets/tuples -> lists
    """
    if obj is None:
        return None
    ...
    if is_dataclass(obj):
        out: Dict[str, Any] = {}
        for f in fields(obj):
            out[f.name] = _jsonify(getattr(obj, f.name))
        return out
