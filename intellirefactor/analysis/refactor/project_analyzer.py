...
from intellirefactor.interfaces import BaseProjectAnalyzer, GenericAnalysisResult
...
class ProjectAnalyzer(BaseProjectAnalyzer):
    ...
    def analyze_file(self, filepath: Path) -> GenericAnalysisResult:
        ...
        try:
            ...
            if lines_count > large_file_threshold:
                issues.append(
                    f"Large file: {lines_count} lines (threshold: {large_file_threshold})"
                )
                recommendations.append("Consider splitting into multiple modules")

            if complexity_score > complexity_threshold:
                issues.append(
                    f"High complexity: {complexity_score:.1f} (threshold: {complexity_threshold})"
                )
                recommendations.append("Extract methods into separate components")

            if responsibilities_count > responsibilities_threshold:
                issues.append(
                    f"Multiple responsibilities: {responsibilities_count} (threshold: {responsibilities_threshold})"
                )
            ...
            for cls in classes:
                ...
                if len(class_methods) > god_object_threshold:
                    issues.append(
                        f"God Object: class {cls.name} has {len(class_methods)} methods (threshold: {god_object_threshold})"
                    )
            ...
            return GenericAnalysisResult(
                ...
                metadata={"analyzer": "ProjectAnalyzer", "version": "1.4.0"},
    ...
    def analyze_project(self, project_path: Union[str, Path]) -> GenericAnalysisResult:
        ...
        try:
            ...
            refactoring_candidates = [
                ...
                and analysis.data.get("lines_count", 0) > min_candidate_size
            ]
            ...
            metrics = {
                ...
                "complex_files_ratio": (
                    len(complex_files) / len(source_files) if source_files else 0.0
                ),
    ...
    def _generate_overall_recommendations(
        self,
        file_analyses: List[GenericAnalysisResult],
...
def create_project_analyzer(
    config: Optional[AnalysisConfig] = None, project_root: str = "."
) -> ProjectAnalyzer:
