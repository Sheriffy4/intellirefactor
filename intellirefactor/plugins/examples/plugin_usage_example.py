...
from intellirefactor.plugins.examples.example_custom_rules_plugin import (
    CustomRulesPlugin,
)
from intellirefactor.plugins.examples.example_refactoring_plugin import (
    ExampleRefactoringPlugin,
)
from intellirefactor.plugins.examples.example_knowledge_plugin import (
...
class PluginUsageExample:
    ...
    def _coordinate_analysis_results(
        ...
        analysis_results["coordinated"] = {
            ...
            "average_metrics": {
                name: sum(values) / len(values) for name, values in merged_metrics.items()
            },
    ...
    def _share_knowledge(
        ...
        if recommendations:
            self.hook_system.set_context(
                "shared_recommendations", recommendations[0] if recommendations else {}
            )

    def _validate_opportunities(
        self, analysis_results: Dict[str, Any], opportunities: List[Dict[str, Any]]
    ) -> None:
        ...
        for opportunity in opportunities:
            ...
            for warning in risk_warnings:
                if warning.get("type") == opportunity_type:
                    ...
                    opportunity["risk_reason"] = warning.get(
                        "warning", "Historical low success rate"
                    )
    ...
    def analyze_project(self, project_path: str) -> Dict[str, Any]:
        ...
        analysis_results = {"project_path": project_path, "files": {}, "plugins": {}}
        ...
        self.hook_system.execute_hooks(
            HookType.POST_PROJECT_ANALYSIS, project_path, analysis_results
        )
    ...
    def identify_refactoring_opportunities(
        ...
        self.hook_system.execute_hooks(
            HookType.POST_OPPORTUNITY_DETECTION, analysis_results, all_opportunities
        )
    ...
    def _generate_workflow_summary(self, workflow_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (
                successful_refactorings / len(refactoring_results) if refactoring_results else 0
            ),
    ...
    try:
        ...
        if results.get("opportunities"):
            ...
            for i, opp in enumerate(results["opportunities"][:3]):  # Show first 3
                logger.info(
                    f"{i + 1}. {opp.get('type', 'unknown')}: {opp.get('description', 'No description')}"
                )
        ...
        if results.get("recommendations"):
            ...
            if recs.get("refactoring_suggestions"):
                ...
                for rec in recs["refactoring_suggestions"][:3]:  # Show first 3
                    logger.info(
                        f"- {rec.get('recommendation', 'No recommendation')} (confidence: {rec.get('confidence', 0):.1%})"
                    )
