...
class HookRegistry:
    ...
    def register_hook(self, hook: Hook) -> bool:
        ...
        hook_list = (
            self._custom_hooks.get(hook.name, [])
            if hook.hook_type == HookType.CUSTOM
        ...
        logger.info(
            f"Registered hook: {hook.name} (type: {hook.hook_type.value}, priority: {hook.priority.value})"
        )
        return True
...
class HookSystem:
    ...
    def register_hook(
        ...
        if name is None:
            name = (
                f"{plugin_name or 'unknown'}_{callback.__name__}_{len(self.registry._hook_names)}"
            )
...
def custom_hook(
    hook_key: str, priority: HookPriority = HookPriority.NORMAL, plugin_name: str = None
):
