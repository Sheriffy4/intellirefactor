...
class ArchitectureGenerator:
    ...
    def analyze_module(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            # IMPORTANT: reset state per module analysis
            self.components = {}
            self.relationships = []
    ...
    def _extract_components(self, tree: ast.AST, source_code: str) -> None:
        ...
        # IMPORTANT:
        # Do NOT use ast.walk(tree) here, because it includes methods inside classes.
        # We want:
        # - classes (top-level)
        ...
        for method, classes in method_signatures.items():
            if len(classes) >= 3 and method not in ["__init__", "__str__", "__repr__"]:
                self.design_patterns.append(
                    f"Strategy Pattern: {method} method implemented by {', '.join(classes)}"
                )
        ...
        for name, component in self.components.items():
            if component.type == "class":
                ...
                if len(component.dependencies) >= 5:
                    self.design_patterns.append(
                        f"Facade Pattern: {name} class coordinates multiple subsystems"
                    )
        ...
        for name, component in self.components.items():
            if component.type == "class" and component.methods:
                observer_methods = [
                    ...
                    if "notify" in m.lower()
                    or "update" in m.lower()
                    or "callback" in m.lower()
                    or "listener" in m.lower()
                ]
                if observer_methods:
                    self.design_patterns.append(
                        f"Observer Pattern: {name} class with {', '.join(observer_methods)}"
                    )
...
"""
        ...
        function_components = {
            name: comp for name, comp in components.items() if comp.type == "function"
        }
...
"""
        ...
        if class_components:
            ...
            for name, component in class_components.items():
                diagram += f"#### {name}\n"
                diagram += (
                    f"- **Purpose**: {component.purpose[:100]}...\n"
                    if len(component.purpose) > 100
                    else f"- **Purpose**: {component.purpose}\n"
                )
    ...
    def generate_data_flow_diagram(self, analysis_result: Dict[str, Any], module_name: str) -> str:
