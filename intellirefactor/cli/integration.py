...
class CLIIntegrationManager:
    ...
    def knowledge_manager(self) -> KnowledgeManager:
        ...
        if self._knowledge_manager is None:
            self._knowledge_manager = KnowledgeManager(self.config.knowledge_settings)
    ...
    def orchestrator(self) -> GlobalRefactoringOrchestrator:
        ...
        if self._orchestrator is None:
            self._orchestrator = GlobalRefactoringOrchestrator(
                project_root=None, dry_run=False, config=self.config.refactoring_settings
            )
    ...
    def reporter(self) -> RefactoringReporter:
        ...
        if self._reporter is None:
            self._reporter = RefactoringReporter(
    ...
    def validator(self) -> RefactoringValidator:
        ...
        if self._validator is None:
            self._validator = RefactoringValidator(
                project_root=None, config=self.config.refactoring_settings
            )
    ...
    def project_analyzer(self) -> ProjectAnalyzer:
        ...
        if self._project_analyzer is None:
            self._project_analyzer = ProjectAnalyzer(self.config.analysis_settings)
    ...
    def metrics_analyzer(self) -> MetricsAnalyzer:
        ...
        if self._metrics_analyzer is None:
            self._metrics_analyzer = MetricsAnalyzer(self.config.analysis_settings)
    ...
    def refactoring_system(self) -> IntelligentRefactoringSystem:
        ...
        if self._refactoring_system is None:
            self._refactoring_system = IntelligentRefactoringSystem(self.config.refactoring_settings)
    ...
    def safety_manager(self) -> SafetyManager:
        ...
        if self._safety_manager is None:
            self._safety_manager = SafetyManager(self.config.refactoring_settings)
        ...
        if include_opportunities:
            ...
            if "project_analysis" in results:
                ...
                if hasattr(project_analysis, "data") and project_analysis.data:
                    ...
                    project_analysis_data = {
                        k: v for k, v in project_analysis.__dict__.items() if not k.startswith("_")
                    }
    ...
    def generate_comprehensive_report(
        ...
        # Convert analysis results to the format expected by the reporter
        refactoring_results = []
            ...
            for opp in analysis_results["opportunities"]:
                ...
                if hasattr(opp, '__dict__'):
                    ...
                    opp_dict = {
                        ...
                        "priority": getattr(opp, "priority", 0),
                        "effort": getattr(opp, "effort", 0.0),
                        "impact": getattr(opp, "impact", {}),
                        "files": getattr(opp, "files", []),
                    }
                else:
                    # Already a dict
                    opp_dict = {
                        ...
                        "effort": opp.get("effort", 0.0),
                        "impact": opp.get("impact", {}),
                        "files": opp.get("files", []),
                    }
                refactoring_results.append(opp_dict)

        # Use the correct method name and format
        report_format = "text" if output_format == "markdown" else output_format
    ...
    def migrate_legacy_data(
        self, legacy_data_path: Path, target_format: str = "modern"
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            migration_results["errors"].append({"error": str(e), "type": type(e).__name__})
    ...
    def _migrate_refactoring_opportunities(
        self, legacy_opportunities: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
    ...
    def _migrate_knowledge_items(
        self, legacy_knowledge: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
            ...
            try:
                ...
                if not status.get("compatible", True):
                    ...
                    compatibility_report["issues"].append(
                        f"{name}: {status.get('issue', 'Unknown compatibility issue')}"
                    )
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "config": {
                "safety_level": (
                    self.config.safety_level.value
                    if hasattr(self.config, "safety_level")
                    else "unknown"
                ),
            ...
            "plugins": {
                "loaded": (
                    len(self.plugin_manager.get_loaded_plugins()) if self._plugin_manager else 0
                ),
                "available": (
                    len(self.plugin_manager.get_available_plugins()) if self._plugin_manager else 0
                ),
            },
        }
