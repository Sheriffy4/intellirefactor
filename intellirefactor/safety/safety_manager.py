...
from .validation_tools import RefactoringValidator
...
class SafetyManager:
    ...
    def pre_refactoring_check(
        self, project_path: str, refactoring_plan: Dict[str, Any]
    ) -> SafetyResult:
        ...
        try:
            ...
            if missing_critical:
                analysis["concerns"].append(
                    f"Missing critical files: {', '.join(missing_critical)}"
                )
            ...
            try:
                ...
                if total_files > 1000:
                    analysis["concerns"].append(
                        f"Large project ({total_files} Python files) - consider incremental refactoring"
                    )
