...
from intellirefactor.analysis.responsibility_clusterer import ResponsibilityClusterer, ClusteringConfig, ClusteringAlgorithm
from intellirefactor.analysis.refactoring_decision_engine import (
    RefactoringDecisionEngine, DecisionCriteria, RefactoringPriority, RefactoringType
)
from intellirefactor.refactoring.llm_context_generator import LLMContextGenerator, ContextType
...
def create_parser() -> argparse.ArgumentParser:
    ...
    analyze_parser = subparsers.add_parser(
        'analyze', 
        help='Analyze project or file for refactoring opportunities'
    )
    ...
    analyze_parser.add_argument(
        '--no-metrics',
        action='store_true',
        help='Skip detailed metrics calculation'
    )
    analyze_parser.add_argument(
        '--no-opportunities',
        action='store_true',
    ...
    analyze_parser.add_argument(
        '--entry-point',
        help='Main script to trace execution flow (e.g., cli.py)'
    )
    analyze_parser.add_argument(
        '--visualize',
        action='store_true',
    ...
    opportunities_parser = subparsers.add_parser(
        'opportunities',
        help='Identify and list refactoring opportunities'
    )
    ...
    opportunities_parser.add_argument(
        '--output', '-o',
        help='Output file for opportunities list'
    )
    opportunities_parser.add_argument(
        '--format',
        choices=['json', 'text'],
    ...
    refactor_parser = subparsers.add_parser(
        'refactor', 
        help='Perform refactoring operations'
    )
    refactor_parser.add_argument(
        'path', 
        help='Path to project directory to refactor'
    )
    refactor_parser.add_argument(
        '--strategy', 
    ...
    refactor_parser.add_argument(
        '--output', '-o', 
        help='Output file for refactoring results'
    )
    refactor_parser.add_argument(
        '--no-validation',
        action='store_true',
    ...
    apply_parser = subparsers.add_parser(
        'apply',
        help='Apply a specific refactoring opportunity'
    )
    ...
    knowledge_parser = subparsers.add_parser(
        'knowledge', 
        help='Query and manage knowledge base'
    )
    ...
    report_parser = subparsers.add_parser(
        'report',
        help='Generate comprehensive refactoring reports'
    )
    report_parser.add_argument(
        'results_file',
        help='JSON file containing refactoring results'
    )
    report_parser.add_argument(
        '--format',
    ...
    status_parser = subparsers.add_parser(
        'status',
        help='Show system status and health information'
    )
    ...
    init_parser.add_argument('--path', default='intellirefactor.json', help='Configuration file path')
    init_parser.add_argument('--template', help='Configuration template to use')
    init_parser.add_argument('--format', choices=['json', 'yaml'], default='json', help='Configuration file format')
    ...
    template_parser = subparsers.add_parser(
        'template',
        help='Configuration template management'
    )
    ...
    # Generate template
    generate_parser = template_subparsers.add_parser('generate', help='Generate configuration from template')
    ...
    generate_parser.add_argument('--safety-level', choices=['conservative', 'moderate', 'aggressive'], 
                                default='moderate', help='Refactoring safety level')
    ...
    show_parser.add_argument('--format', choices=['json', 'yaml'], default='json', help='Display format')
    
    # Index command
    index_parser = subparsers.add_parser(
        'index',
        help='Index management for persistent analysis data'
    )
    ...
    # Index rebuild
    rebuild_parser = index_subparsers.add_parser('rebuild', help='Completely rebuild the index from scratch')
    rebuild_parser.add_argument(
        'project_path',
        help='Path to project directory to rebuild index for'
    )
    rebuild_parser.add_argument(
        '--batch-size', '-b',
        type=int,
    ...
    rebuild_parser.add_argument(
        '--output', '-o',
        help='Output file for rebuild results'
    )
    rebuild_parser.add_argument(
        '--format',
        choices=['json', 'text'],
    ...
    duplicates_parser = subparsers.add_parser(
        'duplicates',
        help='Code clone detection and analysis'
    )
    duplicates_subparsers = duplicates_parser.add_subparsers(dest='duplicates_action', help='Clone detection operations')
    
    # Duplicates blocks
    blocks_parser = duplicates_subparsers.add_parser('blocks', help='Detect block-level code clones')
    blocks_parser.add_argument(
        'project_path',
        help='Path to project directory to analyze for block clones'
    )
    blocks_parser.add_argument(
        '--exact-threshold',
        type=float,
    ...
    blocks_parser.add_argument(
        '--output', '-o',
        help='Output file for clone detection results'
    )
    blocks_parser.add_argument(
        '--format',
    ...
    blocks_parser.add_argument(
        '--show-code',
        action='store_true',
        help='Include code snippets in output'
    )
    blocks_parser.add_argument(
        '--group-by',
    ...
    # Duplicates methods
    methods_parser = duplicates_subparsers.add_parser('methods', help='Detect method-level code clones')
    methods_parser.add_argument(
        'project_path',
        help='Path to project directory to analyze for method clones'
    )
    methods_parser.add_argument(
    ...
    methods_parser.add_argument(
        '--output', '-o',
        help='Output file for method clone detection results'
    )
    methods_parser.add_argument(
        '--format',
        choices=['json', 'text', 'html'],
    ...
    similar_parser = duplicates_subparsers.add_parser('similar', help='Find semantically similar methods')
    ...
    unused_parser = subparsers.add_parser(
        'unused',
        help='Unused code detection and analysis'
    )
    unused_subparsers = unused_parser.add_subparsers(dest='unused_action', help='Unused code detection operations')
    
    # Unused detect
    detect_parser = unused_subparsers.add_parser('detect', help='Detect unused code at multiple levels')
    detect_parser.add_argument(
        'project_path',
        help='Path to project directory to analyze for unused code'
    )
    detect_parser.add_argument(
        '--entry-points',
        nargs='*',
    ...
    detect_parser.add_argument(
        '--filter-type',
        choices=['module_unreachable', 'symbol_unused', 'private_method_unused', 
                'public_export_unused', 'tests_only', 'scripts_only', 'uncertain_dynamic'],
    ...
    detect_parser.add_argument(
        '--output', '-o',
        help='Output file for unused code detection results'
    )
    detect_parser.add_argument(
        '--format',
        choices=['json', 'text', 'html'],
    ...
    audit_parser = subparsers.add_parser(
        'audit',
        help='Comprehensive project audit combining multiple analysis types'
    )
    ...
    audit_parser.add_argument(
        '--json-output',
        help='Output path for JSON artifacts (default: analysis.json)'
    )
    
    # Smells command group
    smells_parser = subparsers.add_parser(
        'smells',
        help='Architectural smell detection and analysis'
    )
    smells_subparsers = smells_parser.add_subparsers(dest='smells_action', help='Smell detection operations')
    
    # Smells detect
    smells_detect_parser = smells_subparsers.add_parser(
        'detect',
        help='Detect architectural smells in project'
    )
    ...
    smells_detect_parser.add_argument(
        '--smell-types',
        nargs='*',
        choices=['god_class', 'long_method', 'high_complexity', 'srp_violation', 'feature_envy', 'inappropriate_intimacy'],
    ...
    smells_detect_parser.add_argument(
        '--output', '-o',
        help='Output file for smell detection results'
    )
    smells_detect_parser.add_argument(
        '--format',
        choices=['json', 'text', 'html'],
    ...
    cluster_parser = subparsers.add_parser(
        'cluster',
        help='Responsibility clustering for component extraction'
    )
    ...
    responsibility_parser.add_argument(
        '--class-name',
        help='Specific class name to analyze (optional)'
    )
    responsibility_parser.add_argument(
        '--file-path',
        help='Specific file path to analyze (optional)'
    )
    responsibility_parser.add_argument(
        '--min-confidence',
        type=float,
    ...
    responsibility_parser.add_argument(
        '--show-plans',
        action='store_true',
        help='Show detailed extraction plans'
    )
    responsibility_parser.add_argument(
        '--group-by',
    ...
    decide_parser = subparsers.add_parser(
        'decide',
        help='Refactoring decision engine for intelligent recommendations'
    )
    decide_subparsers = decide_parser.add_subparsers(dest='decide_action', help='Decision engine operations')
    ...
    decide_analyze_parser.add_argument(
        '--criteria-config',
        help='Path to decision criteria configuration file'
    )
    decide_analyze_parser.add_argument(
        '--min-confidence',
        type=float,
    ...
    decide_analyze_parser.add_argument(
        '--output', '-o',
        help='Output file for decision analysis results'
    )
    decide_analyze_parser.add_argument(
        '--format',
    ...
    decide_recommend_parser = decide_subparsers.add_parser(
        'recommend',
        help='Get specific refactoring recommendations with filtering'
    )
    decide_recommend_parser.add_argument(
        'analysis_file',
        help='Path to decision analysis results file'
    )
    decide_recommend_parser.add_argument(
        '--priority',
        choices=['critical', 'high', 'medium', 'low'],
    ...
    decide_recommend_parser.add_argument(
        '--type',
        choices=['extract_method', 'extract_class', 'remove_unused_code', 
                'decompose_god_class', 'reduce_method_complexity', 'eliminate_duplicates',
                'improve_cohesion', 'reduce_coupling', 'parameterize_duplicates', 
    ...
    decide_plan_parser = decide_subparsers.add_parser(
        'plan',
        help='Generate refactoring sequence and execution plan'
    )
    ...
    generate_parser = subparsers.add_parser(
        'generate',
        help='Generate specification documents from analysis results'
    )
    generate_subparsers = generate_parser.add_subparsers(dest='generate_action', help='Specification generation operations')
    ...
    generate_spec_parser.add_argument(
        '--template',
        help='Custom template file for specification generation'
    )
    generate_spec_parser.add_argument(
        '--include-index',
        action='store_true',
    ...
    generate_spec_parser.add_argument(
        '--include-unused',
        action='store_true',
        help='Include unused code detection'
    )
    generate_spec_parser.add_argument(
        '--min-confidence',
        type=float,
    ...
    system_parser = subparsers.add_parser(
        'system',
        help='System management and integration commands'
    )
    ...
    system_status_parser = system_subparsers.add_parser(
        'status',
        help='Show comprehensive system status with rich output'
    )
    ...
    system_init_parser = system_subparsers.add_parser(
        'init',
        help='Initialize IntelliRefactor for a project with full integration'
    )
    ...
    system_init_parser.add_argument(
        '--load-plugins',
        action='store_true',
        help='Load and initialize plugins'
    )
    
    # System migrate
    system_migrate_parser = system_subparsers.add_parser(
        'migrate',
        help='Migrate legacy data to modern format'
    )
    system_migrate_parser.add_argument(
        'legacy_data_path',
        help='Path to legacy data file or directory'
    )
    system_migrate_parser.add_argument(
        '--output', '-o',
    ...
    visualize_parser = subparsers.add_parser(
        'visualize',
        help='Generate visualizations for code analysis'
    )
    visualize_subparsers = visualize_parser.add_subparsers(dest='visualize_action', help='Visualization operations')
    
    # Visualize method
    method_parser = visualize_subparsers.add_parser('method', help='Generate method flowchart')
    method_parser.add_argument(
        'file_path',
        help='Path to the Python file containing the method'
    )
    method_parser.add_argument(
        'method_name',
        help='Name of the method to visualize'
    )
    method_parser.add_argument(
        '--output', '-o',
    ...
    call_graph_parser = visualize_subparsers.add_parser('call-graph', help='Generate call dependency graph')
    call_graph_parser.add_argument(
        'symbol_name',
        help='Name of the function/method to trace calls for'
    )
    call_graph_parser.add_argument(
        '--max-depth',
        type=int,
    ...
    refactor_enhanced_parser.add_argument(
        'plan_file',
        help='Path to refactoring plan file (JSON)'
    )
    refactor_enhanced_parser.add_argument(
        '--dry-run',
        action='store_true',
    ...
    docs_parser = subparsers.add_parser(
        'docs',
        help='Generate comprehensive documentation for modules'
    )
    docs_subparsers = docs_parser.add_subparsers(dest='docs_action', help='Documentation operations')
    
    # Generate full documentation suite
    full_docs_parser = docs_subparsers.add_parser('generate', help='Generate complete documentation suite')
    full_docs_parser.add_argument(
        'target_file',
        help='Path to Python file to document'
    )
    full_docs_parser.add_argument(
        '--output-dir', '-o',
    ...
    full_docs_parser.add_argument(
        '--include',
        nargs='+',
        choices=['architecture', 'flowchart', 'call_graph', 'report', 'registry', 'llm_context', 'project_structure'],
    ...
    type_docs_parser = docs_subparsers.add_parser('type', help='Generate specific documentation type')
...
def format_refactoring_result(result: RefactoringResult, format_type: str) -> str:
    ...
    if format_type == 'json':
        ...
        output = []
        if result.success:
            output.append(f"Refactoring completed successfully")
...
def cmd_analyze(args, intellirefactor: IntelliRefactor) -> None:
    ...
    if hasattr(args, 'visualize') and args.visualize:
        ...
        # Create orchestrator
        orchestrator = GlobalRefactoringOrchestrator(project_root=path if path.is_dir() else path.parent)
        
        # Prepare visualization config
        viz_config = {'entry_point': args.entry_point} if hasattr(args, 'entry_point') and args.entry_point else {}
        ...
        try:
            ...
            if stage_result.success:
                print(f"✅ Visualizations generated successfully in: {stage_result.details.get('output_dir', 'unknown')}")
            else:
...
def cmd_knowledge(args, intellirefactor: IntelliRefactor) -> None:
    ...
    if args.knowledge_action == 'query':
        ...
        print(f"Knowledge base path: {intellirefactor.config.knowledge_settings.knowledge_base_path}")
...
def cmd_visualize(args) -> None:
    ...
    try:
        if args.visualize_action == 'method':
            ...
            generator = FlowchartGenerator(None)  # We'll create the generator without index for method visualization
            ...
            if args.output:
                ...
                output_path = Path('visualizations') / f"{file_name}_{args.method_name}_flowchart.md"
            ...
            if not db_path.exists():
                ...
                print("Please build the index first using: intellirefactor index build <project_path>", file=sys.stderr)
                sys.exit(1)
            ...
            print(f"Error: Unknown visualize action: {args.visualize_action}", file=sys.stderr)
            sys.exit(1)
...
def cmd_docs(args) -> None:
    ...
    try:
        ...
        if not target_file.suffix == '.py':
            print(f"Error: Target file must be a Python file (.py)", file=sys.stderr)
            sys.exit(1)
    ...
    if args.config_action == 'show':
        ...
        if args.template:
            ...
            try:
                ...
                print(f"Error: Failed to create configuration from template: {e}", file=sys.stderr)
                sys.exit(1)
        else:
    ...
    if format_type == 'json':
        ...
        if detailed and 'detailed_stats' in stats:
            ...
            if 'complexity_distribution' in detailed_stats:
                ...
                output.append(f"High complexity symbols: {complexity.get('high_complexity_count', 0)}")
...
def format_clone_detection_results(clone_groups, statistics, format_type: str, 
                                 show_code: bool = False, group_by: str = 'type') -> str:
    """Format clone detection results for output."""
    from intellirefactor.analysis.block_clone_detector import CloneType, ExtractionStrategy
    ...
    if format_type == 'json':
        ...
        html_parts.append(f'<h1>Clone Detection Results</h1>')
        html_parts.append(f'<p>Found {len(clone_groups)} clone groups with {statistics["total_instances"]} total instances</p>')
        
        for group in clone_groups:
            html_parts.append(f'<div class="clone-group">')
            html_parts.append(f'<h3>{group.clone_type.value.title()} Clone Group</h3>')
            html_parts.append(f'<p>Similarity: {group.similarity_score:.3f}, Instances: {len(group.instances)}</p>')
            
            if group.extraction_strategy and group.extraction_strategy != ExtractionStrategy.NO_EXTRACTION:
                html_parts.append(f'<p><strong>Recommended:</strong> {group.extraction_strategy.value.replace("_", " ").title()} (confidence: {group.extraction_confidence:.2f})</p>')
            
            for instance in group.instances:
                html_parts.append(f'<div class="instance">')
                html_parts.append(f'<strong>{instance.file_path}:{instance.line_start}-{instance.line_end}</strong>')
                html_parts.append(f' ({instance.block_info.lines_of_code} LOC, {instance.block_info.statement_count} statements)')
        ...
        output.append("=" * 50)
        output.append(f"Found {len(clone_groups)} clone groups with {statistics['total_instances']} total instances")
        ...
        for group_key, groups in grouped.items():
            ...
            for group in groups:
                ...
                if group.extraction_strategy and group.extraction_strategy != ExtractionStrategy.NO_EXTRACTION:
                    output.append(f"  Recommended: {group.extraction_strategy.value.replace('_', ' ').title()} (confidence: {group.extraction_confidence:.2f})")
                
                output.append("  Locations:")
                for instance in group.instances:
                    output.append(f"    {instance.file_path}:{instance.line_start}-{instance.line_end} ({instance.block_info.lines_of_code} LOC)")
                
                if show_code and group.instances:
                    # Show code snippet from first instance
                    first_instance = group.instances[0]
                    try:
                        with open(first_instance.file_path, 'r') as f:
                            lines = f.readlines()
...
def format_similarity_results(results, format_type: str, show_evidence: bool = False, 
                            show_differences: bool = False, show_merge_recommendations: bool = False) -> str:
    ...
    if format_type == 'html':
        ...
        if not results:
            ...
            for result in results:
                ...
                output.append(f'<p><strong>File:</strong> {result["target_method"]["file_path"]}</p>')
                output.append(f'<p><strong>Lines:</strong> {result["target_method"]["line_start"]}-{result["target_method"]["line_end"]}</p>')
                ...
                if result['similar_methods']:
                    ...
                    for similar in result['similar_methods']:
                        ...
                        output.append(f'Lines: {similar["method"]["line_start"]}-{similar["method"]["line_end"]}<br>')
                        ...
                        if show_merge_recommendations and similar.get('merge_recommendation'):
                            ...
                            output.append(f'<br><strong>Merge Strategy:</strong> {rec["strategy"]}<br>')
    ...
    for result in results:
        ...
        output.append(f"  Lines: {result['target_method']['line_start']}-{result['target_method']['line_end']}")
        output.append("")
        ...
        if result['similar_methods']:
            ...
            for i, similar in enumerate(result['similar_methods'], 1):
                ...
                output.append(f"     Lines: {similar['method']['line_start']}-{similar['method']['line_end']}")
    ...
    if args.index_action == 'build':
        ...
        try:
            ...
            def progress_callback(progress: float, files_processed: int, total_files: int):
                if total_files > 0:
                    print(f"\rProgress: {progress:.1f}% ({files_processed}/{total_files} files)", end='', flush=True)
    ...
    try:
        if args.duplicates_action == 'blocks':
            ...
            output = format_clone_detection_results(
                clone_groups, detector.get_clone_statistics(clone_groups),
                args.format, args.show_code, args.group_by
            )
            ...
            if args.target_method:
                target_method = next(
                    (m for m in methods if m.qualified_name == args.target_method),
                    None
                )
            ...
            if target_method:
                ...
                for match in matches:
                    similar_method = match.method2 if match.method1 == target_method else match.method1
                    similar_methods.append({
                        ...
                        } if match.merge_strategy else None
                    })
...
def format_unused_code_results(analysis_result, format_type: str, show_evidence: bool = False, 
                              show_usage: bool = False, group_by: str = 'type') -> str:
    """Format unused code detection results for output."""
    from intellirefactor.analysis.unused_code_detector import UnusedCodeType
    ...
    if format_type == 'json':
        ...
        html_parts.append(f'<h1>Unused Code Detection Results</h1>')
        ...
        for finding in analysis_result.findings:
            ...
            html_parts.append(f'<h3>{finding.unused_type.value.replace("_", " ").title()}: {finding.symbol_name}</h3>')
            html_parts.append(f'<p><strong>File:</strong> {finding.file_path}:{finding.line_start}-{finding.line_end}</p>')
            html_parts.append(f'<p><strong>Confidence:</strong> {finding.confidence:.3f} ({finding.confidence_level.value})</p>')
        ...
        output.append(f"Analysis confidence threshold: {analysis_result.analysis_metadata.get('confidence_threshold', 'N/A')}")
        output.append("")
        ...
        for group_key, findings in grouped.items():
            ...
            for finding in findings:
                ...
                output.append(f"  Location: {finding.file_path}:{finding.line_start}-{finding.line_end}")
                output.append(f"  Confidence: {finding.confidence:.3f} ({finding.confidence_level.value})")
                ...
                if show_usage and finding.usage_references:
                    ...
                    for ref in finding.usage_references[:3]:  # Show first 3
                        output.append(f"    {ref.file_path}:{ref.line_number} ({ref.pattern.value})")
                ...
                if finding.dynamic_usage_indicators:
                    output.append(f"  Dynamic usage indicators: {len(finding.dynamic_usage_indicators)}")
...
def cmd_unused(args) -> None:
    ...
    from intellirefactor.analysis.unused_code_detector import UnusedCodeDetector, UnusedCodeType
    ...
    try:
        if args.unused_action == 'detect':
            ...
            if args.level != 'all':
                level_filters = {
                    '1': [UnusedCodeType.MODULE_UNREACHABLE, UnusedCodeType.TESTS_ONLY, UnusedCodeType.SCRIPTS_ONLY],
                    '2': [UnusedCodeType.SYMBOL_UNUSED, UnusedCodeType.PRIVATE_METHOD_UNUSED, UnusedCodeType.PUBLIC_EXPORT_UNUSED],
...
def format_audit_results(audit_result, format_type: str) -> str:
    ...
    from intellirefactor.analysis.audit_models import AuditSeverity, AuditFindingType
    ...
    if format_type == 'json':
        ...
        html_parts.append(f'<h1>Project Audit Results</h1>')
        ...
        html_parts.append(f'<p><strong>Files Analyzed:</strong> {audit_result.statistics.files_analyzed}</p>')
        html_parts.append(f'<p><strong>Analysis Time:</strong> {audit_result.statistics.analysis_time_seconds:.2f}s</p>')
        ...
        for severity in [AuditSeverity.CRITICAL, AuditSeverity.HIGH, AuditSeverity.MEDIUM, AuditSeverity.LOW]:
            ...
            html_parts.append(f'<h2>{severity.value.title()} Priority ({len(severity_findings)} findings)</h2>')
            
            for finding in severity_findings:
                html_parts.append(f'<div class="finding {severity.value}">')
                html_parts.append(f'<h3>{finding.title}</h3>')
                html_parts.append(f'<p><strong>Type:</strong> {finding.finding_type.value.replace("_", " ").title()}</p>')
                html_parts.append(f'<p><strong>Location:</strong> {finding.file_path}:{finding.line_start}-{finding.line_end}</p>')
        ...
        for severity, count in stats.findings_by_severity.items():
            if count > 0:
                emoji = {'critical': '[CRITICAL]', 'high': '[HIGH]', 'medium': '[MEDIUM]', 'low': '[LOW]', 'info': '[INFO]'}.get(severity, '')
        ...
        for severity in [AuditSeverity.CRITICAL, AuditSeverity.HIGH, AuditSeverity.MEDIUM, AuditSeverity.LOW]:
            ...
            emoji = {'critical': '[CRITICAL]', 'high': '[HIGH]', 'medium': '[MEDIUM]', 'low': '[LOW]'}.get(severity.value, '')
            output.append(f"{emoji} {severity.value.title()} Priority Findings ({len(severity_findings)}):")
            output.append("-" * 40)
            
            for finding in severity_findings:
                output.append(f"  {finding.title}")
                output.append(f"  Type: {finding.finding_type.value.replace('_', ' ')}")
                output.append(f"  Location: {Path(finding.file_path).name}:{finding.line_start}-{finding.line_end}")
        ...
        if len(audit_result.findings) > 0:
            ...
            if critical_count > 0:
                output.append(f"  1. [CRITICAL] Address {critical_count} critical issues immediately")
            if high_count > 0:
...
def cmd_audit(args) -> None:
    ...
    try:
        ...
        print(f"Audit completed: {len(result.findings)} findings in {result.statistics.analysis_time_seconds:.2f}s")
        ...
        if args.emit_spec and not result.analysis_metadata.get('spec_generated'):
            ...
            if args.spec_output:
                ...
                if spec_base_path.suffix == '.md':
                    ...
                    implementation_content = spec_generator.generate_implementation_from_audit(result)
        ...
        if args.emit_json:
            spec_generator = SpecGenerator()
            json_path = Path(args.json_output) if args.json_output else project_path / 'analysis.json'
...
def cmd_smells(args) -> None:
    ...
    from intellirefactor.analysis.architectural_smell_detector import (
        ArchitecturalSmellDetector, SmellThresholds, SmellType, SmellSeverity
    )
    import glob
    import time
        ...
        for file_path in python_files:
            try:
                ...
                if args.severity:
                    ...
                    severity_order = [SmellSeverity.LOW, SmellSeverity.MEDIUM, SmellSeverity.HIGH, SmellSeverity.CRITICAL]
...
def format_smells_json(smells, args, analysis_time, processed_files):
    ...
    for smell in smells:
        ...
        result['statistics']['by_severity'][severity] = result['statistics']['by_severity'].get(severity, 0) + 1
        result['statistics']['by_type'][smell_type] = result['statistics']['by_type'].get(smell_type, 0) + 1
...
def cmd_cluster(args) -> None:
    ...
    if args.cluster_action == 'responsibility':
        ...
        for file_path in python_files:
            try:
                ...
                for class_node in classes:
                    ...
                    result = clusterer.cluster_class_methods(source_code, class_node.name, str(file_path))
        ...
        print(f"Clustering analysis completed: {len(all_results)} classes analyzed in {analysis_time:.2f}s")
...
def format_clustering_results_text(results, suggestions, args):
    ...
    for result in results:
        ...
        output.append(f"Unclustered Methods: {len(result.unclustered_methods)} ({result.unclustered_ratio:.1%})")
        ...
        if result.clusters:
            ...
            for i, cluster in enumerate(result.clusters, 1):
                ...
                output.append(f"     Responsibilities: {', '.join(cluster.dominant_responsibilities)}")
                ...
                if cluster.shared_dependencies:
                    output.append(f"     Shared Dependencies: {', '.join(cluster.shared_dependencies)}")
        ...
        for suggestion in suggestions:
            ...
            if args.show_interfaces:
                ...
                if interface.external_dependencies:
                    output.append(f"  External Dependencies: {', '.join(interface.external_dependencies)}")
...
def format_clustering_results_json(results, suggestions, args):
    ...
    def serialize_result(result):
        return {
            ...
            'unclustered_methods': [{'name': m.name, 'line_start': m.line_start} for m in result.unclustered_methods],
    ...
    output = {
        ...
        'component_suggestions': [serialize_suggestion(s) for s in suggestions] if args.show_suggestions else [],
        'summary': {
...
def cmd_decide_analyze(args) -> None:
    ...
    try:
        ...
        if args.export_decisions:
            ...
            with open(args.export_decisions, 'w') as f:
                f.write(exported)
...
def format_recommendations_text(decisions: List[Dict], include_evidence: bool = False, 
                               include_plans: bool = False) -> str:
    ...
    for i, decision in enumerate(decisions, 1):
        ...
        if include_plans and decision.get('implementation_plan'):
            ...
            for step in decision['implementation_plan'][:3]:  # Show first 3 steps
                lines.append(f"     {step.get('step_number', '?')}. {step.get('title', 'Untitled step')}")
...
def format_plan_text(plan_data: Dict) -> str:
    ...
    for i, decision in enumerate(plan_data.get('decisions', []), 1):
        lines.append(f"{i}. {decision.get('title', 'Untitled')}")
        lines.append(f"   Priority: {decision.get('priority', 'unknown')}")
        lines.append(f"   Effort: {decision.get('feasibility', {}).get('estimated_hours', 1.0):.1f} hours")
...
def cmd_generate_spec(args) -> None:
    ...
    try:
        ...
        if args.spec_type == 'all':
            ...
            requirements_content = spec_generator.generate_requirements_from_audit(audit_result, custom_template)
            ...
            design_content = spec_generator.generate_design_from_audit(audit_result, custom_template)
            ...
            implementation_content = spec_generator.generate_implementation_from_audit(audit_result, custom_template)
            ...
            content = spec_generator.generate_specification(args.spec_type, audit_result, custom_template)
        ...
        if audit_result.statistics.total_findings > 0:
            print(f"\nNext Steps:")
            print(f"1. Review the generated specifications")
            print(f"2. Prioritize refactoring tasks based on findings")
            print(f"3. Begin implementation following the generated plan")
        else:
...
def cmd_system_status(args, rich_output: RichOutputManager) -> None:
    ...
    try:
        rich_output.print_header("IntelliRefactor System Status", "Comprehensive system health and integration check")
        ...
        components_table = rich_output.create_table("Component Status", ["Component", "Status", "Loaded"])
        for component, loaded in system_status['components'].items():
            status = "✓ Ready" if loaded else "○ Not loaded"
            rich_output.add_table_row(components_table, component.replace('_', ' ').title(), status, str(loaded))
        ...
        if getattr(args, 'show_plugins', False) or system_status['plugins']['loaded'] > 0:
            ...
            rich_output.add_table_row(plugins_table, "Loaded Plugins", system_status['plugins']['loaded'])
            rich_output.add_table_row(plugins_table, "Available Plugins", system_status['plugins']['available'])
        ...
        if compatibility_report:
            ...
            if compatibility_report['overall_status'] == 'compatible':
                rich_output.print_success(f"All components are compatible")
            elif compatibility_report['overall_status'] == 'incompatible':
                rich_output.print_warning(f"Some compatibility issues found")
            else:
                rich_output.print_error(f"Compatibility check failed")
...
def cmd_system_init(args, rich_output: RichOutputManager) -> None:
    ...
    try:
        ...
        rich_output.print_header("Project Initialization", f"Setting up IntelliRefactor for {project_path}")
        ...
        rich_output.print_success(f"Project initialized successfully")
        ...
        rich_output.add_table_row(init_table, "Knowledge Base", "✓ Initialized" if init_result['knowledge_status'] else "○ Skipped")
        rich_output.add_table_row(init_table, "Plugins Loaded", str(init_result['plugins_loaded']))
        rich_output.add_table_row(init_table, "Safety Systems", "✓ Initialized" if init_result['safety_initialized'] else "○ Failed")
...
def cmd_system_migrate(args, rich_output: RichOutputManager) -> None:
    ...
    try:
        ...
        if migration_result['errors']:
            rich_output.print_warning(f"Migration completed with {len(migration_result['errors'])} errors")
        ...
        for item in migration_result['migrated_items']:
            rich_output.add_table_row(migration_table, item['type'].replace('_', ' ').title(), str(item['count']))
...
def cmd_analyze_enhanced(args) -> None:
    ...
    try:
        ...
        rich_output.print_header("Enhanced Project Analysis", f"Comprehensive analysis of {project_path}")
        ...
        if getattr(args, 'interactive', False):
            ...
            include_metrics = rich_output.confirm("Include detailed metrics analysis?", default=True)
            include_opportunities = rich_output.confirm("Include refactoring opportunities?", default=True)
        ...
        with progress:
            analysis_task = rich_output.add_progress_task(progress, "Running comprehensive analysis", 100)
        ...
        if include_opportunities and 'opportunities' in analysis_result:
            ...
            if opportunities:
                ...
                opp_table = rich_output.create_table("Top Opportunities", ["Title", "Priority", "Effort", "Impact"])
...
def cmd_refactor_enhanced(args) -> None:
    ...
    try:
        ...
        rich_output.print_header("Enhanced Refactoring", f"Executing refactoring plan for {project_path}")
        ...
        if getattr(args, 'interactive', False):
            ...
            rich_output.print_info(f"Plan contains {len(refactoring_plan.get('steps', []))} refactoring steps")
        ...
        with progress:
            refactor_task = rich_output.add_progress_task(progress, "Processing refactoring steps", 100)
        ...
        if execution_result['backup_info']:
            rich_output.print_info(f"Backup created: {execution_result['backup_info']['backup_path']}")
