...
class KnowledgeManager:
    ...
    def _recalculate_statistics(self, index: Dict[str, Any]) -> None:
        ...
        refactoring_files = [
            f for f in index["knowledge_files"] if f["type"] == "refactoring_metadata"
        ]
        ...
        avg_reusability = sum(f.get("reusability_score", 0) for f in refactoring_files) / len(
            refactoring_files
        )
            ...
            if file_info["type"] == "refactoring_metadata":
                ...
                if context in applicable_contexts:
                    recommendations.append(
                        f"Используйте паттерны из {file_info['project_name']} "
