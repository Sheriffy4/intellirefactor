...
class AutoRefactor:
    ...
    def analyze_god_object(self, filepath: Path) -> RefactoringPlan:
        ...
        estimated_effort = (
            len(extracted_components) * 2.5 + 4
        )  # базовые 4 часа + по 2.5 на компонент
    ...
    def generate_component_interface(
        self, component_name: str, methods: List[ast.FunctionDef]
    ) -> str:
...
'''
    ...
    def generate_component_implementation(
        self, component_name: str, methods: List[ast.FunctionDef], original_content: str
    ) -> str:
    ...
    def _extract_method_code(self, method_node: ast.FunctionDef, original_content: str) -> str:
        ...
        if method_lines:
            ...
            if non_empty_lines:
                ...
                method_lines = [
                    line[min_indent:] if len(line) > min_indent else line for line in method_lines
                ]
...
'''
        ...
        for component in components:
            ...
            facade_code += (
                f"from .services.{service_name}_service import {component}, I{component}\n"
            )
    ...
    def execute_refactoring(
        ...
        try:
            ...
            for group_name, methods in method_groups.items():
                ...
                impl_code = self.generate_component_implementation(
                    component_name, methods, original_content
                )
...
def main():
    ...
    parser.add_argument(
        "--knowledge-base",
        default="knowledge/refactoring_automation_metadata.json",
