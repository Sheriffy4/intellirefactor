...
class ProjectDoctor:
    ...
    def build_dependency_graph(self, python_files: List[Path]):
        ...
        for file_path in python_files:
            ...
            local_imports = {
                imp for imp in imports if imp.startswith(("core", "ml", "tests", "recon"))
            }
                    ...
                    for method in node.body:
                        if isinstance(method, ast.FunctionDef) and method.name == "__init__":
                            ...
                            if missing_deps:
                                ...
                                self.report.add_import_error(f"{class_name} (DI)", error_msg)
                            else:
                                self.report.add_successful_import(f"{class_name} (DI compliant)")
                            break
                    break
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ClassDef) and node.name == class_name:
                    ...
                    if missing_methods:
                        error_msg = f"Missing interface methods: {', '.join(missing_methods)}"
                        self.report.add_import_error(f"{class_name} ({interface_name})", error_msg)
                    else:
