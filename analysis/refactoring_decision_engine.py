...
try:
    ...
    from .architectural_smell_detector import ArchitecturalSmell, SmellType, SmellSeverity
...
class DecisionCriteria:
    ...
    def __post_init__(self):
        ...
        total_weight = (self.code_quality_weight + self.performance_weight + 
                       self.architecture_weight + self.risk_weight + self.effort_weight)
...
class ImpactAssessment:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.evidence:
            result['evidence'] = (
                self.evidence.to_dict() if hasattr(self.evidence, 'to_dict') 
...
class FeasibilityAnalysis:
    ...
    def __post_init__(self):
        for score_name in ['effort_score', 'risk_score', 'complexity_score']:
            ...
            if val < 0.0: setattr(self, score_name, 0.0)
            elif val > 1.0: setattr(self, score_name, 1.0)
...
class RefactoringDecision:
    ...
    def __post_init__(self):
        ...
        elif self.confidence > 1.0: self.confidence = 1.0
    ...
    def get_overall_impact_score(self) -> float:
        ...
        if not self.impact_assessments:
            return 0.0
        return sum(assessment.score for assessment in self.impact_assessments) / len(self.impact_assessments)
...
class DecisionAnalysisResult:
    ...
    def get_high_confidence_decisions(self, min_confidence: float = 0.8) -> List[RefactoringDecision]:
...
class RefactoringDecisionEngine:
    ...
    def __init__(self, criteria: Optional[DecisionCriteria] = None):
        ...
        logger.info(f"RefactoringDecisionEngine initialized")
    ...
    def analyze_project(self, audit_result: Any) -> DecisionAnalysisResult:
        ...
        try:
            ...
            smell_types = {'god_class', 'long_method', 'high_complexity', 
                          'srp_violation', 'feature_envy', 'inappropriate_intimacy'}
        ...
        result = DecisionAnalysisResult(
            ...
            analysis_metadata={
                ...
                'analysis_sources': {
                    ...
                    'unused_findings': len(getattr(getattr(audit_result, 'unused_result', None), 'findings', []))
                }
            }
        )
    ...
    def _analyze_clone_groups(self, clone_groups: List[Any], analysis_run_id: str) -> List[RefactoringDecision]:
        ...
        for clone_group in clone_groups:
            ...
            for inst in instances:
                ...
                if path: target_files.append(path)
            ...
            try:
                if _MODELS_AVAILABLE and Evidence is not None:
                    ...
                    file_refs = []
                    for inst in instances:
                        if hasattr(inst, 'block_info') and hasattr(inst.block_info, 'file_reference'):
                ...
                logger.warning(f"Could not generate full evidence for clone group: {e}", exc_info=True)
    ...
    def _analyze_architectural_smells(self, smell_findings: List[Any], analysis_run_id: str) -> List[RefactoringDecision]:
        ...
        for finding in smell_findings:
            try:
                ...
                refactoring_type = self.SMELL_REFACTORING_MAP.get(finding_type_val, RefactoringType.IMPROVE_COHESION)
                
                # Impact calculation
                impact_score = 0.5
                if severity_val == 'critical': impact_score = 0.9
                elif severity_val == 'high': impact_score = 0.7
                elif severity_val == 'low': impact_score = 0.3
                ...
                if finding_evidence:
                    evidence = [finding_evidence] if not isinstance(finding_evidence, list) else finding_evidence
                else:
    ...
    def _calculate_smell_feasibility(self, finding: Any) -> FeasibilityAnalysis:
        ...
        effort = 0.5
        if sev_val == 'critical': effort = 0.8
        elif sev_val == 'low': effort = 0.2
    ...
    def _create_smell_remediation_plan(self, finding: Any) -> List[ImplementationStep]:
        ...
        return [
            ImplementationStep(1, "Analyze smell", f"Understand impact of {finding_type}", "30 minutes"),
