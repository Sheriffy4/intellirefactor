...
class FileAnalyzer(BaseFileAnalyzer):
    ...
    def __init__(self, config: Optional[AnalysisConfig] = None):
        ...
        self.responsibility_keywords: Dict[str, List[str]] = {
            ...
            "security": ["auth", "secure", "encrypt", "decrypt", "hash", "token", "login"],
            "network": ["connect", "request", "response", "http", "tcp", "socket", "fetch", "url"],
            "storage": ["store", "save", "load", "persist", "retrieve", "cache", "db", "repo"],
            ...
            "transformation": ["transform", "convert", "modify", "change", "update", "map"],
            "coordination": ["coordinate", "orchestrate", "manage", "control", "handle", "dispatch"],
    ...
    def analyze_file(
        ...
        try:
            ...
            complexity_score = sum(c.total_complexity for c in classes) + sum(f.complexity for f in functions)

            # Nested defs: separate, do NOT affect main metrics/issues/priority
            nested_classes, nested_functions, nested_summary = self._collect_nested_definitions(tree)

            issues = self._identify_issues(lines_count, classes, functions, complexity_score, nested_summary)
            ...
            analysis_data: Dict[str, Any] = {"legacy_result": legacy_result, **legacy_result}
    ...
    def _iter_nodes_excluding_nested(self, node: python_ast.AST) -> Iterable[python_ast.AST]:
        ...
        for child in python_ast.iter_child_nodes(node):
            ...
            if not isinstance(
                child,
                (python_ast.FunctionDef, python_ast.AsyncFunctionDef, python_ast.ClassDef, python_ast.Lambda),
            ):
    ...
    def _analyze_method(self, node: Union[python_ast.FunctionDef, python_ast.AsyncFunctionDef]) -> MethodInfo:
        complexity = self._calculate_complexity(node)
        parameters_count = self._count_parameters(node)
        calls_count = sum(1 for n in self._iter_nodes_excluding_nested(node) if isinstance(n, python_ast.Call))
    ...
    def _calculate_complexity(self, node: python_ast.AST) -> int:
        ...
        for child in self._iter_nodes_excluding_nested(node):
            if isinstance(child, (python_ast.If, python_ast.While, python_ast.For, python_ast.AsyncFor)):
                complexity += 1
            elif isinstance(child, python_ast.BoolOp):
                complexity += max(0, len(child.values) - 1)
            elif isinstance(child, python_ast.Try):
                complexity += len(child.handlers)
            elif isinstance(child, (python_ast.ListComp, python_ast.SetComp, python_ast.DictComp, python_ast.GeneratorExp)):
    ...
    def _count_parameters(self, node: Union[python_ast.FunctionDef, python_ast.AsyncFunctionDef]) -> int:
        args = node.args
    ...
    def _identify_issues(
        ...
        if lines_count > cfg.large_file_threshold * 2:
            issues.append(f"Very large file: {lines_count} lines (recommended <{cfg.large_file_threshold})")
        elif lines_count > cfg.large_file_threshold:
            issues.append(f"Large file: {lines_count} lines (recommended <{cfg.large_file_threshold})")

        file_complexity_limit = self.config.complexity_threshold * 10
        if complexity_score > file_complexity_limit * 2:
            issues.append(f"Critical complexity: {complexity_score} (recommended <{file_complexity_limit})")
        elif complexity_score > file_complexity_limit:
            issues.append(f"High complexity: {complexity_score} (recommended <{file_complexity_limit})")
    ...
    def _generate_recommendations(
        ...
        if has_god_object:
            recs.append("Extract Component: split God Objects into smaller single-responsibility classes")
        ...
        if has_deep_nesting:
            recs.append("Flatten Structure: move deeply nested classes/functions to module level where possible")

        if has_many_nested:
            recs.append("Extract Nested: promote frequently-used nested definitions to module level")
        ...
        if automation >= 0.7:
            recs.append(f"Automation potential is high ({automation:.0%}): consider automated refactoring tools")
        elif automation >= 0.4:
            recs.append(f"Automation potential is medium ({automation:.0%}): partial automation is feasible")
    ...
    def _enrich_with_context(self, filepath: Path, context: Any, data: Dict[str, Any], metadata: Dict[str, Any]) -> None:
    ...
    def generate_detailed_report(self, result: GenericAnalysisResult) -> str:
        ...
        if functions:
            ...
            for fn in functions:
                ...
                report.append(f"- {icon} **{async_marker}{fn.get('name', 'Unknown')}**: complexity {cplx}")
