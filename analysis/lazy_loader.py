...
class LazyProjectContext:
    ...
    def __init__(self, project_root: Path, index_store: IndexStore, 
    ...
    def _with_cache(self, cache_name: str, key: str, loader: Callable[[], Any], 
    ...
    def get_file_info(self, file_path: str) -> Optional[Dict[str, Any]]:
        ...
        if self.config.enable_caching:
            return self._with_cache('_file_cache', file_path, _load_file_info, 
    ...
    def get_symbol_info(self, symbol_uid: str) -> Optional[Dict[str, Any]]:
        ...
        if self.config.enable_caching:
            return self._with_cache('_symbol_cache', symbol_uid, _load_symbol_info,
                                  self.config.cache_size_limit)
        else:
    ...
    def find_importers_of_module(self, module_path: str) -> List[Dict[str, Any]]:
        ...
        if self.config.enable_caching:
            return self._with_cache('_dependency_cache', cache_key, _load_importers,
                                  self.config.cache_size_limit)
        else:
    ...
    def find_usage_of_symbol(self, symbol_uid: str) -> List[Dict[str, Any]]:
        ...
        if self.config.enable_caching:
            return self._with_cache('_dependency_cache', cache_key, _load_symbol_usage,
                                  self.config.cache_size_limit)
        else:
    ...
    def is_symbol_used_in_project(self, symbol_uid: str) -> bool:
        ...
        if self.config.enable_caching:
            return self._with_cache('_dependency_cache', cache_key, _check_symbol_usage,
                                  self.config.cache_size_limit)
        else:
    ...
    def lazy_load_project_structure(self) -> Dict[str, Any]:
        ...
        if (self.config.lazy_file_scanning and 
            len(python_files) <= self.config.max_project_files_for_full_load):
    ...
    def get_cached_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'total_cache_size': (len(self._file_cache) + 
                               len(self._symbol_cache) + 
                               len(self._dependency_cache))
        }
...
class LazyContextManager:
    ...
    def get_context(self, project_root: Path, index_store: IndexStore,
                   config: Optional[LazyLoadConfig] = None) -> LazyProjectContext:
        """
