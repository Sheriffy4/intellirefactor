...
from .lazy_loader import LazyProjectContext, LazyContextManager
...
class UnusedCodeDetector:
    ...
    def detect_unused_code(self, 
                          ...
                          external_index: Optional['IndexStore'] = None) -> UnusedCodeAnalysisResult:
        """
        ...
        findings.extend(self._detect_unreachable_modules(python_files, entry_points, external_index))
    ...
    def _discover_python_files(self, include_patterns: List[str], exclude_patterns: List[str]) -> List[Path]:
                    ...
                    if isinstance(node, ast.Call):
                        if isinstance(node.func, ast.Name):
                            ...
                            self._add_usage_reference(
                                symbol_name, str(file_path), node.lineno,
    ...
    def _detect_unreachable_modules(self, python_files: List[Path], entry_points: List[str], external_index: Optional['IndexStore'] = None) -> List[UnusedCodeFinding]:
        ...
        for file_path in python_files:
            ...
            if not is_reachable_from_entry_points and not is_used_in_external_project:
                ...
                is_script = any(re.search(pattern, str(file_path)) for pattern in self.script_patterns)
    ...
    def _detect_unused_symbols(self, python_files: List[Path], external_index: Optional['IndexStore'] = None) -> List[UnusedCodeFinding]:
        ...
        for symbol_key, symbol_info in defined_symbols.items():
            ...
            external_usage = [ref for ref in usage_refs if ref.file_path != symbol_info['file_path']]
            ...
            if not external_usage and not is_used_in_external_project:
                ...
                dynamic_import_indicators = [indicator for indicator in dynamic_indicators 
        ...
        for file_path in python_files:
            ...
            if dynamic_indicators:
                ...
                if dynamic_import_indicators:
                    ...
                    evidence = Evidence(
                        description=f"File contains dynamic usage patterns that may hide code usage",
    ...
    def _add_usage_reference(self, symbol_name: str, file_path: str, line_number: int,
                           pattern: UsagePattern, context: str, confidence: float) -> None:
    ...
    def _generate_statistics(self, findings: List[UnusedCodeFinding], python_files: List[Path]) -> Dict[str, Any]:
        ...
        for finding in findings:
            ...
            stats['findings_by_type'][unused_type] = stats['findings_by_type'].get(unused_type, 0) + 1
        ...
        for node in ast.walk(tree):
            if (isinstance(node, ast.Assign) and 
                ...
                for elt in node.value.elts:
                    ...
                    elif hasattr(ast, 'Constant') and isinstance(elt, ast.Constant) and isinstance(elt.value, str):  # Python 3.8+
    ...
    def _is_module_used_in_external_index(self, module_name: str, external_index: 'IndexStore') -> bool:
    ...
    def _is_symbol_used_in_external_index(self, symbol_name: str, file_path: str, external_index: 'IndexStore') -> bool:
