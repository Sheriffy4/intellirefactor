...
from .debug_cycle_manager import DebugCycleManager, DebugProgress, DebugCycle, Issue, Fix
...
class IncrementalDebuggingWorkflow:
    ...
    def create_checkpoint(self, description: str = "") -> str:
        ...
        try:
            ...
            checkpoint_id = f"{self.current_progress.workflow_id}_{int(datetime.now().timestamp() * 1000)}"
            ...
            checkpoint = WorkflowCheckpoint(
                ...
                metadata={'description': description, 'operation_count': self.operation_count}
            )
    ...
    def execute_debug_operation(
        ...
        try:
            ...
            if result.get('success', False):
                ...
                self.current_progress.operations_completed.append(f"{operation_type}:{self.operation_count}")
            else:
            ...
            logger.info(f"Debug operation {operation_type} completed: {result.get('success', False)}")
            return result
    ...
    def validate_workflow_state(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.current_progress:
                if self.current_progress.completed_steps + self.current_progress.failed_steps > self.current_progress.total_steps:
            ...
            debug_progress = self.debug_manager.get_debug_progress()
            if debug_progress:
                if debug_progress.successful_cycles + debug_progress.failed_cycles > debug_progress.total_cycles:
            ...
            self.validation_state['validation_count'] = self.validation_state.get('validation_count', 0) + 1
            ...
            for checkpoint_file in self.checkpoints_dir.glob(pattern):
                try:
                    ...
                    checkpoint_info = {
                        ...
                        'operation_count': checkpoint_data.get('metadata', {}).get('operation_count', 0),
    ...
    def _execute_issue_detection(self, operation_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not error:
                return {'success': False, 'error': 'No error provided for issue detection'}
            
            # Use debug manager to detect issues
            issues = self.debug_manager.detect_intellirefactor_issues(error, context, additional_info)
    ...
    def _execute_fix_application(self, operation_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not issue_data:
                return {'success': False, 'error': 'No issue provided for fix application'}
            
            # Convert dict to Issue object if needed
            if isinstance(issue_data, dict):
                from .debug_cycle_manager import IssueType, ErrorSeverity
                issue = Issue(
                    issue_id=issue_data['issue_id'],
                    issue_type=IssueType[issue_data['issue_type']],
                    severity=ErrorSeverity(issue_data['severity']),
    ...
    def _execute_debug_cycle(self, operation_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            successful_fixes = sum(1 for result in cycle.validation_results 
