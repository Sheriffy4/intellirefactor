...
from .models import DeepMethodInfo, Evidence, SemanticCategory, ResponsibilityMarker
...
class SemanticSimilarityMatcher:
    ...
    def find_similar_methods(self, methods: List[DeepMethodInfo],
                           target_method: Optional[DeepMethodInfo] = None,
                           similarity_types: Optional[Set[SimilarityType]] = None) -> List[SimilarityMatch]:
        """
        ...
        similarity_types = similarity_types or {SimilarityType.STRUCTURAL, SimilarityType.FUNCTIONAL, SimilarityType.BEHAVIORAL}
        matches = []
    ...
    def _compare_methods(self, method1: DeepMethodInfo, method2: DeepMethodInfo,
                        similarity_types: Set[SimilarityType]) -> Optional[SimilarityMatch]:
        ...
        # Generate evidence
        evidence = self._generate_similarity_evidence(method1, method2, similarities, primary_type, confidence)
    ...
    def _jaccard_similarity_empty_as_unknown(self, set1: Set, set2: Set, empty_score: float = 0.0) -> float:
        """Calculate Jaccard similarity, treating empty sets as unknown/no match."""
        if not set1 and not set2:
            return empty_score
        return self._jaccard_similarity(set1, set2)

    def _calculate_structural_similarity(self, method1: DeepMethodInfo, method2: DeepMethodInfo) -> float:
    ...
    def _calculate_functional_similarity(self, method1: DeepMethodInfo, method2: DeepMethodInfo) -> float:
        ...
        ops1 = {op.strip().lower() for op in seq1.operations if op and op.strip().lower() != 'unknown'}
        ops2 = {op.strip().lower() for op in seq2.operations if op and op.strip().lower() != 'unknown'}
    ...
    def _calculate_behavioral_similarity(self, method1: DeepMethodInfo, method2: DeepMethodInfo) -> float:
        ...
        return (category_score * w_cat + resp_score * w_resp + side_effect_score * w_se) / (w_cat + w_resp + w_se)
    ...
    def _extract_operation_sequence(self, method: DeepMethodInfo) -> OperationSequence:
        ...
        if method.operation_signature:
            operations = [op.strip().lower() for op in method.operation_signature.split('->') if op.strip()]
    ...
    def _identify_operation_pattern(self, operations: List[str]) -> OperationPattern:
        ...
        if ('validate' in ops_set or 'check' in ops_set) and ('transform' in ops_set or 'process' in ops_set):
             return OperationPattern.VALIDATE_TRANSFORM_RETURN
        
        return OperationPattern.UNKNOWN
    
    def _calculate_match_confidence(self, method1: DeepMethodInfo, method2: DeepMethodInfo,
                                  similarities: Dict[SimilarityType, float]) -> float:
        ...
        if method1.cyclomatic_complexity > 0 and method2.cyclomatic_complexity > 0:
            complexity_ratio = min(method1.cyclomatic_complexity, method2.cyclomatic_complexity) / \
        ...
        if method1.lines_of_code > 0 and method2.lines_of_code > 0:
            size_ratio = min(method1.lines_of_code, method2.lines_of_code) / \
                        max(method1.lines_of_code, method2.lines_of_code)
    ...
    def _generate_similarity_evidence(self, method1: DeepMethodInfo, method2: DeepMethodInfo,
        ...
        file_refs = [ref for ref in [method1.file_reference, method2.file_reference] if ref is not None]
    ...
    def _analyze_method_differences(self, method1: DeepMethodInfo, method2: DeepMethodInfo) -> Tuple[List[str], List[str]]:
        ...
        if abs(method1.cyclomatic_complexity - method2.cyclomatic_complexity) > 2:
            differences.append(f"Complexity: {method1.name}={method1.cyclomatic_complexity}, {method2.name}={method2.cyclomatic_complexity}")
        
        # Size differences
        if abs(method1.lines_of_code - method2.lines_of_code) > 5:
            differences.append(f"Size: {method1.name}={method1.lines_of_code} LOC, {method2.name}={method2.lines_of_code} LOC")
    ...
    def _suggest_merge_strategy(self, method1: DeepMethodInfo, method2: DeepMethodInfo,
                              similarity_type: SimilarityType, similarity_score: float) -> str:
        ...
        if similarity_score >= 0.9:
            if similarity_type == SimilarityType.STRUCTURAL:
                ...
                return "Full merge candidate - methods are highly similar across multiple dimensions"
    ...
    def get_similarity_statistics(self, matches: List[SimilarityMatch]) -> Dict[str, Any]:
        ...
        return {
            ...
            'methods_involved': len(set(m.method1.qualified_name for m in matches) | 
