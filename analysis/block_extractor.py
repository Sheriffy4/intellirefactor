...
class BlockExtractor:
    ...
    def generate_fingerprints(self, node: ast.AST, source_lines: List[str], 
                            start_line: int, end_line: int) -> BlockFingerprints:
    ...
    def _extract_ast_structure(self, node: ast.AST) -> str:
        ...
        if isinstance(node, ast.Module):
            return "Module(" + ",".join(self._extract_ast_structure(child) for child in node.body) + ")"
        elif isinstance(node, ast.FunctionDef):
            return f"FunctionDef({len(node.args.args)}," + ",".join(self._extract_ast_structure(child) for child in node.body) + ")"
        elif isinstance(node, ast.ClassDef):
            return "ClassDef(" + ",".join(self._extract_ast_structure(child) for child in node.body) + ")"
        elif isinstance(node, ast.If):
            return "If(" + ",".join(self._extract_ast_structure(child) for child in node.body + node.orelse) + ")"
        elif isinstance(node, ast.For):
            return "For(" + ",".join(self._extract_ast_structure(child) for child in node.body + node.orelse) + ")"
        elif isinstance(node, ast.While):
            return "While(" + ",".join(self._extract_ast_structure(child) for child in node.body + node.orelse) + ")"
        elif isinstance(node, ast.Try):
            return "Try(" + ",".join(self._extract_ast_structure(child) for child in node.body + node.orelse + node.finalbody) + ")"
        elif isinstance(node, ast.With):
            return "With(" + ",".join(self._extract_ast_structure(child) for child in node.body) + ")"
    ...
    def _normalize_source(self, source_code: str) -> str:
        ...
        def replace_variable(match):
            ...
            if var_name in {'if', 'else', 'elif', 'for', 'while', 'try', 'except', 'finally',
                           'def', 'class', 'return', 'yield', 'import', 'from', 'as',
                           'True', 'False', 'None', 'and', 'or', 'not', 'in', 'is',
                           'print', 'len', 'str', 'int', 'float', 'list', 'dict', 'set',
                           'STRING_LITERAL', 'NUMBER_LITERAL'}:  # Also skip our replacements
                return var_name
    ...
    def _calculate_lines_of_code(self, start_line: int, end_line: int, 
