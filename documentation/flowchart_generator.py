...
class FlowchartGenerator:
    ...
    def _find_method(self, tree: ast.AST, method_name: str) -> Optional[ast.FunctionDef]:
        """Find a specific method in the AST."""
    ...
    def _analyze_method_flow(self, method_node: ast.FunctionDef, lines: List[str]) -> None:
        ...
        start_node = FlowNode(
            id=start_id,
            type="start",
            content=f"Start: {method_node.name}",
        ...
        if not any(node.type == "end" for node in self.nodes.values()):
            ...
            end_node = FlowNode(
                ...
                type="end",
                content="End",
                line_number=method_node.end_lineno or method_node.lineno
            )
    ...
    def _analyze_statement(self, stmt: ast.stmt, prev_node_id: str, lines: List[str]) -> str:
    ...
    def _analyze_if_statement(self, stmt: ast.If, prev_node_id: str, lines: List[str]) -> str:
        ...
        if stmt.body:
            ...
            first_true_id = self._find_or_create_statement_node(first_true_stmt, lines)
        ...
        if stmt.orelse:
            ...
            # Add false path
            first_else_stmt = stmt.orelse[0]
            first_else_id = self._find_or_create_statement_node(first_else_stmt, lines)
    ...
    def _analyze_for_loop(self, stmt: ast.For, prev_node_id: str, lines: List[str]) -> str:
        """Analyze for loop."""
    ...
    def _analyze_while_loop(self, stmt: ast.While, prev_node_id: str, lines: List[str]) -> str:
        """Analyze while loop."""
    ...
    def _analyze_try_statement(self, stmt: ast.Try, prev_node_id: str, lines: List[str]) -> str:
    ...
    def _analyze_return_statement(self, stmt: ast.Return, prev_node_id: str, lines: List[str]) -> str:
    ...
    def _analyze_raise_statement(self, stmt: ast.Raise, prev_node_id: str, lines: List[str]) -> str:
    ...
    def _analyze_simple_statement(self, stmt: ast.stmt, prev_node_id: str, lines: List[str]) -> str:
