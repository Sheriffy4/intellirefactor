        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                ...
                if self._is_protocol_or_interface(node):
                    interface_info = {
                        'name': node.name,
                        'type': 'protocol' if 'Protocol' in [base.id for base in node.bases if isinstance(base, ast.Name)] else 'interface',
...
"""
    ...
    def _generate_component_registry(self, components: List[ComponentInfo]) -> str:
        ...
        if classes:
            ...
            for cls in classes:
                ...
                registry += f"| `{cls.name}` | {cls.purpose[:50]}... | {cls.complexity} | {deps} |\n"
    ...
    def _generate_method_registry(self, methods: List[MethodInfo]) -> str:
        ...
        for class_name, class_method_list in class_methods.items():
            ...
            for method in class_method_list:
                method_type = "Constructor" if method.name == "__init__" else "Public" if method.is_public else "Private"
    ...
    def _generate_performance_registry(self, components: List[ComponentInfo], 
    ...
    def _generate_testing_registry(self, components: List[ComponentInfo], 
                                 methods: List[MethodInfo]) -> str:
        ...
        for component in components:
            if len(component.dependencies) > 0:
                mock_complexity = "High" if len(component.dependencies) > 5 else "Medium" if len(component.dependencies) > 2 else "Low"
    ...
    def _generate_documentation_registry(self, components: List[ComponentInfo], 
                                       methods: List[MethodInfo]) -> str:
        """Generate documentation registry section."""
        registry = "## Documentation Registry\n\n"
        
        # Calculate documentation coverage
        documented_components = len([c for c in components if c.purpose != "No description available"])
        documented_methods = len([m for m in methods if m.purpose != "No description available"])
        
        total_components = len(components)
        total_methods = len(methods)
        
        component_coverage = (documented_components / total_components * 100) if total_components > 0 else 0
    ...
    def _classify_dependency(self, dep_name: str) -> str:
        ...
        standard_libs = {
            'os', 'sys', 'json', 'logging', 'pathlib', 'time', 'datetime',
            'asyncio', 'subprocess', 'traceback', 'collections', 'dataclasses',
            'enum', 'typing', 're', 'inspect', 'ast'
        }
