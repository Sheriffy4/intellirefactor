...
class ConfigurationManager:
    ...
    def load_env_config() -> Dict[str, Any]:
        ...
        if os.getenv('INTELLIREFACTOR_BACKUP_ENABLED'):
            refactoring['backup_enabled'] = os.getenv('INTELLIREFACTOR_BACKUP_ENABLED').lower() == 'true'
        
        if os.getenv('INTELLIREFACTOR_VALIDATION_REQUIRED'):
            refactoring['validation_required'] = os.getenv('INTELLIREFACTOR_VALIDATION_REQUIRED').lower() == 'true'
        ...
        if os.getenv('INTELLIREFACTOR_CONFIDENCE_THRESHOLD'):
            try:
                knowledge['confidence_threshold'] = float(os.getenv('INTELLIREFACTOR_CONFIDENCE_THRESHOLD'))
            except ValueError:
    ...
    def validate_config(config_data: Dict[str, Any]) -> None:
            ...
            if 'metrics_thresholds' in analysis:
                ...
                for key, value in thresholds.items():
                    if not isinstance(value, (int, float)) or value < 0:
                        raise ConfigurationError(f"metrics_thresholds.{key} must be a non-negative number")
        ...
        if 'refactoring' in config_data:
            ...
            if 'max_operations_per_session' in refactoring and refactoring['max_operations_per_session'] <= 0:
...
class AnalysisConfig:
    ...
    metrics_thresholds: Dict[str, float] = field(default_factory=lambda: {
...
class IntelliRefactorConfig:
    ...
    def load(cls, config_path: Optional[str] = None, 
             use_env: bool = True, 
             validate: bool = True) -> 'IntelliRefactorConfig':
        """
