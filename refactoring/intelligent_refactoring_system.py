...
class CodeAnalyzer:
    ...
    def analyze_file(self, file_path: str) -> CodeMetrics:
        ...
        try:
            # ИСПРАВЛЕНО: заменено 'utf-8' на 'utf-8-sig' для поддержки BOM
            ...
            # Логируем, но не крашим весь процесс
    ...
    def _estimate_responsibilities(self, tree: ast.AST, content: str) -> int:
        ...
        responsibility_keywords = [
            'generate', 'analyze', 'test', 'cache', 'config', 'metric', 
            'monitor', 'validate', 'process', 'handle', 'manage'
        ]
...
class OpportunityDetector:
    ...
    def detect_opportunities(self, file_path: str, context: RefactoringContext) -> List[RefactoringOpportunity]:
    ...
    def _is_god_class(self, metrics: CodeMetrics) -> bool:
        ...
        return (metrics.file_size > thresholds["file_size_threshold"] and 
                metrics.number_of_responsibilities > thresholds["responsibility_threshold"] and
                metrics.cohesion_level < thresholds["cohesion_threshold"])
    ...
    def _is_large_configuration(self, file_path: str, metrics: CodeMetrics) -> bool:
        ...
        thresholds = self.knowledge_base.detection_rules.get(
            ...
            {"file_size_threshold": 200, "responsibility_threshold": 4, "filename_patterns": ["config", "settings"]}
        )
        filename_matches = any(pattern in file_path.lower() for pattern in thresholds["filename_patterns"])
        return (filename_matches and 
                metrics.file_size > thresholds["file_size_threshold"] and
                metrics.number_of_responsibilities > thresholds["responsibility_threshold"])
...
class QualityAssessor:
    ...
    def assess_refactoring_quality(self, 
                                  refactoring_id: str,
                                  before_files: List[str],
                                  after_files: List[str]) -> RefactoringQualityAssessment:
    ...
    def _calculate_improvements(self, before: Dict[str, float], after: Dict[str, float]) -> Dict[RefactoringQualityMetric, float]:
        """Calculate improvement scores for each quality metric."""
        improvements = {}
        
        # Complexity reduction (lower is better)
        if before.get("complexity", 0) > 0:
            complexity_reduction = (before["complexity"] - after.get("complexity", 0)) / before["complexity"]
            improvements[RefactoringQualityMetric.COMPLEXITY_REDUCTION] = max(0.0, complexity_reduction)
            
        # Coupling reduction (lower is better)
        if before.get("avg_coupling", 0) > 0:
            coupling_reduction = (before["avg_coupling"] - after.get("avg_coupling", 0)) / before["avg_coupling"]
        ...
        if before.get("avg_cohesion", 0) > 0:
            cohesion_improvement = (after.get("avg_cohesion", 0) - before["avg_cohesion"]) / (1.0 - before["avg_cohesion"])
            improvements[RefactoringQualityMetric.COHESION_IMPROVEMENT] = max(0.0, cohesion_improvement)
            
        # Maintainability improvement (based on file size and complexity)
        before_maintainability = 1.0 / (1.0 + before.get("avg_file_size", 1000) / 1000.0 + before.get("complexity", 10) / 10.0)
        after_maintainability = 1.0 / (1.0 + after.get("avg_file_size", 1000) / 1000.0 + after.get("complexity", 10) / 10.0)
        maintainability_improvement = (after_maintainability - before_maintainability) / (1.0 - before_maintainability)
        improvements[RefactoringQualityMetric.MAINTAINABILITY_IMPROVEMENT] = max(0.0, maintainability_improvement)
...
class IntelligentRefactoringSystem(BaseRefactoringSystem):
    ...
    def __init__(self, config: Optional[Any] = None):
        # Handle both dict and RefactoringConfig objects
        if hasattr(config, '__dict__'):
            # Convert RefactoringConfig to dict
            self.config = {
    ...
    def _create_knowledge_base(self) -> MachineReadableKnowledgeBase:
        ...
        # Create detection rules (configurable)
        ...
        # Create quality benchmarks (configurable)
        quality_benchmarks = self.config.get("quality_benchmarks", {
        ...
        return MachineReadableKnowledgeBase(
            ...
            transformation_rules=[],  # Would be populated from configuration
            refactoring_patterns=[],  # Would be populated from configuration
            decision_criteria={},  # Would be populated from configuration
    ...
    def analyze_project(self, project_path: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "high_priority_opportunities": len([o for o in all_opportunities if o.severity == "high"]),
            "automation_ready": len([o for o in all_opportunities if o.can_auto_refactor]),
            "estimated_total_effort_hours": sum(o.estimated_effort_hours for o in all_opportunities),
    ...
    def _prioritize_opportunities(self, opportunities: List[RefactoringOpportunity]) -> List[RefactoringOpportunity]:
        ...
        def priority_score(opp: RefactoringOpportunity) -> float:
            ...
            score = (severity_score * opp.confidence * 
    ...
    def _generate_refactoring_plan(self, opportunities: List[RefactoringOpportunity]) -> List[Dict[str, Any]]:
    ...
    def identify_opportunities(self, analysis_data: Dict[str, Any]) -> List[GenericRefactoringOpportunity]:
        ...
        # Extract project path from analysis data
        project_path = analysis_data.get('project_path', '.')
        
        # If we have legacy analysis data, use it
        if 'legacy_analysis' in analysis_data:
            legacy_data = analysis_data['legacy_analysis']
            refactoring_candidates = legacy_data.get('refactoring_candidates', [])
            
            for candidate in refactoring_candidates:
                # Convert to generic opportunity
                opportunity = GenericRefactoringOpportunity(
                    id=f"candidate_{candidate.get('filepath', '').replace('/', '_')}",
                    type="file_refactoring",
                    priority=candidate.get('refactoring_priority', 5),
                    description=f"Refactor {candidate.get('filepath', 'file')} with {len(candidate.get('issues', []))} issues",
                    target_files=[candidate.get('filepath', '')],
                    estimated_impact={
                        'complexity_reduction': 0.3,
                        'maintainability_improvement': 0.4,
                        'automation_potential': candidate.get('automation_potential', 0.5)
                    },
                    prerequisites=[],
                    automation_confidence=candidate.get('automation_potential', 0.5),
                    risk_level="medium"
                )
                opportunities.append(opportunity)
        else:
            # Use the existing analyze_project method
            try:
                analysis_result = self.analyze_project(project_path)
                prioritized_opportunities = analysis_result.get('prioritized_opportunities', [])
                
                for opp in prioritized_opportunities:
                    # Convert RefactoringOpportunity to GenericRefactoringOpportunity
                    generic_opp = GenericRefactoringOpportunity(
                        id=opp.opportunity_id,
                        # ИСПРАВЛЕНО: opp.type вместо opp.opportunity_type (из предыдущего фикса)
                        type=opp.type.value if hasattr(opp.type, 'value') else str(opp.type),
                        priority=int(opp.confidence * 10),
                        description=opp.description,
                        
                        # ИСПРАВЛЕНО: opp.file_path обернут в список, так как target_files не существует
                        target_files=[opp.file_path], 
                        
                        estimated_impact={
                            'complexity_reduction': 0.4,
                            'maintainability_improvement': 0.5,
                            'automation_potential': opp.automation_confidence
                        },
                        # ИСПРАВЛЕНО: используем manual_steps_required вместо prerequisites, если prerequisites нет
                        prerequisites=getattr(opp, 'manual_steps_required', []),
                        automation_confidence=opp.automation_confidence,
                        risk_level=opp.severity
                    )
                    opportunities.append(generic_opp)
                    
            except Exception as e:
                logger.error(f"Failed to analyze project for opportunities: {e}")
                # Добавим traceback для отладки, если нужно
    ...
    def generate_refactoring_plan(self, opportunities: List[GenericRefactoringOpportunity]) -> Dict[str, Any]:
        ...
        This method implements the generic RefactoringSystemProtocol interface.
        """
        # Convert generic opportunities back to internal format for planning
        internal_opportunities = []
        
        for opp in opportunities:
            # Create a simplified RefactoringOpportunity for internal use
            internal_opp = type('RefactoringOpportunity', (), {
                'opportunity_id': opp.id,
                'opportunity_type': opp.type,
                'severity': opp.risk_level,
                'confidence': opp.automation_confidence,
                'automation_confidence': opp.automation_confidence,
                'estimated_effort_hours': max(1, 10 - opp.priority),  # Convert priority to effort
                'target_files': opp.target_files,
                'description': opp.description,
                'prerequisites': opp.prerequisites
            })()
            internal_opportunities.append(internal_opp)
        
        # Use existing planning logic
        plan_items = self._generate_refactoring_plan(internal_opportunities)
        
        return {
            'id': f"plan_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
            'opportunities_count': len(opportunities),
            'estimated_total_effort': sum(max(1, 10 - opp.priority) for opp in opportunities),
            'steps': plan_items,
            'created_at': datetime.now().isoformat(),
            'automation_ready_count': len([opp for opp in opportunities if opp.automation_confidence > 0.7])
        }
